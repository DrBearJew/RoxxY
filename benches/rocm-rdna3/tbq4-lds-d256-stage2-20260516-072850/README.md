...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|:---:|---|---|---|
| `tbq4_lds_route_d_k_d256` | first_runtime_experiment_qwen | `GGML_CUDA_TBQ4_LDS_ROUTE=D_K` | 256 | 48 | 2 | 144 | 130 | 808 | 37.9 KiB | yes/yes/yes | tau0 `1e-6` | `tbq4_vec` | stage2_runtime_target_locked |
| `tbq4_lds_route_d_k_d128` | secondary_dev_shape | `GGML_CUDA_TBQ4_LDS_ROUTE=D_K` | 128 | 96 | 2 | 80 | 66 | 424 | 39.8 KiB | yes/yes/yes | tau0 `1e-6` | `tbq4_vec` | stage1_contract_locked |
| `tbq4_lds_route_b_k_d256` | backup_diagnostic_qwen | `GGML_CUDA_TBQ4_LDS_ROUTE=B_K` | 256 | 48 | 0 | 144 | 130 | 520 | 24.4 KiB | yes/yes/yes | tau0 `1e-6` | `tbq4_vec` | backup_contract_locked |

Route log contracts:
- `route=tbq4_lds_route_d_k_d256 env=GGML_CUDA_TBQ4_LDS_ROUTE=D_K d=256 tile_rows=48 f16_stride=130 packed_stride=144 sparse_v_tau_level=0 fallback=tbq4_vec`
...
- `route=tbq4_lds_route_b_k_d256 env=GGML_CUDA_TBQ4_LDS_ROUTE=B_K d=256 tile_rows=48 f16_stride=130 packed_stride=direct_global sparse_v_tau_level=0 fallback=tbq4_vec`
...
| LDS D-lite K-only D128 | yes | yes | secondary dev-shape contract: ping-pong packed TBQ4 K staging + padded f16 K tile; V unchanged | `GGML_CUDA_TBQ4_LDS_ROUTE=D_K`, tile_rows=96, f16_stride=66 | default-off secondary shape |
...
| LDS D-lite K-only D256 | yes | yes | first Qwen runtime contract: ping-pong packed TBQ4 K staging + padded f16 K tile; V unchanged | `GGML_CUDA_TBQ4_LDS_ROUTE=D_K`, tile_rows=48, f16_stride=130 | Stage 2 implementation target; default-off |
| LDS B-lite K-only D256 | yes | yes | backup diagnostic: direct TBQ4 K -> padded f16 K tile without ping-pong | `GGML_CUDA_TBQ4_LDS_ROUTE=B_K`, tile_rows=48, f16_stride=130 | backup only if D-lite needs diagnostic fallback |
...
| 2 | `tbq4_lds_route_d_k_d256` | 256 | stage2_first_runtime_experiment | Qwen3.6 27B/35B heads are D256; Route D-lite K-only uses ping-pong packed TBQ4 K staging plus one padded f16/half2 K tile, V unchanged | Stage 2/3 default-off implementation for GGML_CUDA_TBQ4_LDS_ROUTE=D_K with Qwen route log, fallback proof, and canaries |
| 3 | `tbq4_lds_route_b_k_d256` | 256 | stage2_backup_diagnostic | backup B-lite D256 K-only isolates whether direct f16 K materialization helps without ping-pong complexity | implement only if D-lite fails from complexity or profiler needs simpler diagnostic comparison |
...
| 5 | `tbq4_lds_route_b` | 128 | model_before_code | single f16 materialization can fit up to 128 rows under 64KiB in this simple model, but scratch/padding/occupancy are not modeled yet | generic route only; concrete Qwen backup is tbq4_lds_route_b_k_d256 |
| 5 | `tbq4_lds_route_b` | 256 | model_before_code | single f16 materialization can fit up to 64 rows under 64KiB in this simple model, but scratch/padding/occupancy are not modeled yet | generic route only; concrete Qwen backup is tbq4_lds_route_b_k_d256 |
| 6 | `tbq4_lds_route_d` | 128 | model_before_code | raw ping-pong plus one f16 tile may overlap loads without doubling f16 LDS cost, but scheduling/barrier cost is unknown | generic route only; concrete Qwen first experiment is tbq4_lds_route_d_k_d256 |
| 6 | `tbq4_lds_route_d` | 256 | model_before_code | raw ping-pong plus one f16 tile may overlap loads without doubling f16 LDS cost, but scheduling/barrier cost is unknown | generic route only; concrete Qwen first experiment is tbq4_lds_route_d_k_d256 |
| 7 | `tbq4_lds_route_d_k_d128` | 128 | secondary_dev_shape | D128 remains useful for non-Qwen/dev canaries but is not the Qwen runtime target | keep default-off and validate only as secondary shape coverage |
...
1. **Stage 1 contract gate**: calculator rows now identify `tbq4_lds_route_d_k_d256` as the first Qwen runtime experiment; D128 remains secondary/dev-shape.
2. **Stage 2 D-lite implementation**: add `GGML_CUDA_TBQ4_LDS_ROUTE=D_K` for D256 K-only with fallback to `tbq4_vec` for env-absent/unsupported shapes.
...
5. **Stage 5 expansion only after D256 success**: optional V integration with sparse-V tau0 pre-dequant skip; Route C remains negative-control only.
