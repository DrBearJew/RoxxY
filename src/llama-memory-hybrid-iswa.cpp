...
void llama_memory_hybrid_iswa::state_write(llama_io_write_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) const {
    ...
    if ((flags & LLAMA_STATE_SEQ_FLAGS_ATTENTION_ONLY) == 0) {
...
bool llama_memory_hybrid_iswa::seq_import_attn_physical(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, size_t * bytes_copied, size_t * cells_copied, const char ** reason) {
