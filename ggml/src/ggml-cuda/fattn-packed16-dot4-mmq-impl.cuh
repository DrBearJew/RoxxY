...
void llama_kv_cache_get_mtp_qblock_tail_page_map(const void * k_view_data,
...
bool ggml_cuda_packed16_dot4_mmq_supported(const int cc, const ggml_tensor * dst) {
    ...
    const char * trace_env = getenv("GGML_CUDA_ROCM_PACKED16_DOT4_MMQ_SUPPORT_TRACE");
    const bool trace = trace_env && atoi(trace_env) != 0;
    const auto reject = [&](const char * reason) {
        if (trace) {
            const int32_t fa_inst = dst ? ((const int32_t *) dst->op_params)[4] : -1;
            fprintf(stderr,
                "PDMQ_SUPPORT reject=%s node=%s cc=%d inst=%d route_required=%d "
                "Q=%s[%lld,%lld,%lld,%lld] K=%s[%lld,%lld,%lld,%lld] "
                "V=%s[%lld,%lld,%lld,%lld] dst=%s[%lld,%lld,%lld,%lld] mask=%s[%lld,%lld,%lld,%lld]\n",
                reason,
                ...
                dst ? (long long) dst->ne[0] : -1LL, dst ? (long long) dst->ne[1] : -1LL,
                dst ? (long long) dst->ne[2] : -1LL, dst ? (long long) dst->ne[3] : -1LL,
                mask ? ggml_type_name(mask->type) : "-",
                mask ? (long long) mask->ne[0] : -1LL, mask ? (long long) mask->ne[1] : -1LL,
                mask ? (long long) mask->ne[2] : -1LL, mask ? (long long) mask->ne[3] : -1LL);
        }
        return false;
    };
    ...
    if (Q->type != GGML_TYPE_F32 || !(k_persistent_i32 || k_f16_hotcold_sidecar) || dst->type != GGML_TYPE_F32) {
        return reject("type_or_k_contract");
    }
    if (Q->ne[0] != PDMQ_D || V->ne[0] != PDMQ_D || dst->ne[0] != PDMQ_D) {
        return reject("d_mismatch");
    }
...
static __device__ __forceinline__ bool pdmq_mask_keep_and_bias(
        ...
        const int implicit_n_kv,
        const int implicit_q_offset,
        const int implicit_flags) {
    *bias = 0.0f;
...
static __device__ __forceinline__ void pdmq_input_probe_hashes(
    ...
    for (int k = 0; k < nk; ++k) {
        ...
        float bias = 0.0f;
        if (valid && (mask || (implicit_flags & 1))) {
            valid = pdmq_mask_keep_and_bias(mask, mask_ne00, mask_ne01, mask_ne03, mask_nb00, mask_nb01, mask_nb03, q, k, b, &bias, implicit_n_kv, implicit_q_offset, implicit_flags);
        }
...
static __device__ __forceinline__ void pdmq_scalar_ref_range(
    ...
    for (int k = k_begin; k < k_end; ++k) {
        ...
        if (valid && (mask || (implicit_flags & 1))) {
            valid = pdmq_mask_keep_and_bias(mask, mask_ne00, mask_ne01, mask_ne03, mask_nb00, mask_nb01, mask_nb03, q, k, b, &mask_bias, implicit_n_kv, implicit_q_offset, implicit_flags);
        }
        if (!valid) {
            continue;
        }
...
static __global__ __launch_bounds__(PDMQ_THREADS, 1) void packed16_dot4_mmq_gqa2_kernel(
        ...
        int implicit_n_kv, int implicit_q_offset, int implicit_flags,
        const int  * __restrict__ k_payload,
        const half * __restrict__ k_scales,
    ...
    for (int k0 = 0; k0 < nk; k0 += BN) {
        ...
        if ((implicit_flags & 1) != 0 && k0 >= implicit_n_kv) {
            break;
        }
...
#pragma unroll
    ...
    if (qpack_payload && qpack_scales) {
        ...
        if (qblock_program.qpack_layout == DP16_FA_QBLOCK_QPACK_LAYOUT_QBLOCK_MAJOR_VEC4) {
            static_assert(Q_WORDS % 4 == 0, "QPACK vector payload load requires int4-aligned rows");
            static_assert(Q_BLOCKS % 4 == 0, "QPACK vector scale load requires float4-aligned rows");
            constexpr int Q_WORD_VECS  = Q_WORDS / 4;
            constexpr int Q_SCALE_VECS = Q_BLOCKS / 4;
            // QPACK rows are 256B payload + 32B scales for D=256, naturally int4 /
            // float4 aligned. Keep split-K unchanged, but load each global QPACK row
            // as vectors and scatter to the padded LDS layout used by the QK dotter.
            // This preserves the +1 LDS stride/bank-padding while cutting consumer
            // setup load instructions and per-word row address arithmetic.
            for (int idx = tid; idx < GQA_GROUP * BM * Q_WORD_VECS; idx += int(blockDim.x)) {
                const int slot = idx / (BM * Q_WORD_VECS);
                const int rem  = idx - slot * BM * Q_WORD_VECS;
                const int qr   = rem / Q_WORD_VECS;
                const int v    = rem - qr * Q_WORD_VECS;
                const int hq   = dp16_fa_qblock_program_hq(qblock_program, hq_base, slot);
                const int q    = dp16_fa_qblock_program_q(qblock_program, q0, qr);
                ...
                const int4 qv = *((const int4 *) (qpack_payload + row * size_t(Q_WORDS) + size_t(v * 4)));
                q_tile_i32[slot][qr][v * 4 + 0] = qv.x;
                q_tile_i32[slot][qr][v * 4 + 1] = qv.y;
                q_tile_i32[slot][qr][v * 4 + 2] = qv.z;
                q_tile_i32[slot][qr][v * 4 + 3] = qv.w;
            }
            for (int idx = tid; idx < GQA_GROUP * BM * Q_SCALE_VECS; idx += int(blockDim.x)) {
                const int slot = idx / (BM * Q_SCALE_VECS);
                const int rem  = idx - slot * BM * Q_SCALE_VECS;
                const int qr   = rem / Q_SCALE_VECS;
                const int v    = rem - qr * Q_SCALE_VECS;
                const int hq   = dp16_fa_qblock_program_hq(qblock_program, hq_base, slot);
                const int q    = dp16_fa_qblock_program_q(qblock_program, q0, qr);
                if (!dp16_fa_qblock_program_head_live(qblock_program, hq, slot, hk, n_heads_q, gqa_ratio) ||
                        !dp16_fa_qblock_program_row_live(qblock_program, q, qr, nq)) {
                ...
                const size_t row = (size_t(b) * size_t(n_heads_q) + size_t(hq)) * size_t(nq) + size_t(q);
                const float4 sv = *((const float4 *) (qpack_scales + row * size_t(Q_BLOCKS) + size_t(v * 4)));
                q_tile_scales[slot][qr][v * 4 + 0] = sv.x;
                q_tile_scales[slot][qr][v * 4 + 1] = sv.y;
                q_tile_scales[slot][qr][v * 4 + 2] = sv.z;
                q_tile_scales[slot][qr][v * 4 + 3] = sv.w;
            }
        } else {
...
#pragma unroll
    ...
    for (int k0 = k_begin; k0 < k_end; k0 += BN) {
        ...
        const bool v4_144_pv4_words_active = v4_144_pv4_active && ((k0 & 7) == 0) &&
            !dp16_fa_qblock_tail_page_consumer_active(qblock_program);
        ...
        if constexpr (STAGE_V) {
            for (int v_idx = tid; v_idx < BN * D; v_idx += int(blockDim.x)) {
                ...
                v_tile[kk * (D + 1) + d] = kk < tile_n && k_phys >= 0
                    ? pdmq_decode_v<V_TYPE>(V, v_nb10, v_nb11, v_nb12, v_nb13, v_ne13, k_phys, hk, b, d, V4_tail, v4_tail_nb10, v4_tail_nb11, v4_tail_nb12, v4_live_nk)
                    : 0.0f;
            }
            __syncthreads();
            ...
            if constexpr (RAW_Q4_LDS) {
                if (!(PV_I4_WMMA && v_i4_cache_words != nullptr && v_i4_cache_scales != nullptr)) {
                    for (int v_idx = tid; v_idx < tile_n * V_BLOCKS; v_idx += int(blockDim.x)) {
                        ...
                        const int k = k0 + kk;
                        const int k_phys = dp16_fa_qblock_tail_page_physical_k(qblock_program, k);
                        v_raw_tile[kk * V_BLOCKS + blk] = k_phys >= 0 ? *pdmq_v_q4_0_block_ptr(
                            V, v_nb10, v_nb11, v_nb12, v_nb13, v_ne13, k_phys, hk, b, blk) : block_q4_0{};
                    }
                    __syncthreads();
                    if constexpr (V4_K16D16_ORACLE) {
                        pdmq_pack_v_q4_0_k16d16_oracle_tile<D, BN>(v_raw_tile, v4_k16d16_words, tile_n);
                ...
                for (int v_idx = tid; v_idx < tile_n * V_SCALE_BLOCKS; v_idx += int(blockDim.x)) {
                    ...
                    const int k = k0 + kk;
                    const int k_phys = dp16_fa_qblock_tail_page_physical_k(qblock_program, k);
                    const int k16_base = k_phys >= 0 ? (k_phys & ~(PDMQ_V4_K16D16_144_K - 1)) : 0;
                    const int slot = k_phys >= 0 ? (k_phys & (PDMQ_V4_K16D16_144_K - 1)) : 0;
                    const char * block = k_phys >= 0 ? pdmq_v_v4_k16d16_144_block_ptr(V, v_nb11, v_nb12, v_nb13, v_ne13, k16_base, hk, b) : nullptr;
                    v4_144_scale_tile[kk * V_SCALE_BLOCKS + d32] = block ? ((const half *) (block + PDMQ_V4_K16D16_144_PAYLOAD_BYTES))[d32 * PDMQ_V4_K16D16_144_K + slot] : __float2half(0.0f);
                }
...
#pragma unroll
                    for (int kg = 0; kg < BN / 8; ++kg) {
                        ...
                        if (v4_144_pv4_words_active && kk < tile_n) {
                            ...
                            const int slot = k_phys >= 0 ? (k_phys & (PDMQ_V4_K16D16_144_K - 1)) : 0;
                            const char * block = k_phys >= 0 ? pdmq_v_v4_k16d16_144_block_ptr(V, v_nb11, v_nb12, v_nb13, v_ne13, k16_base, hk, b) : nullptr;
                            v4_144_pv4_payload_words[kg] = block ? pdmq_v_v4_k16d16_144_payload_word(block, slot, tid) : 0;
                        } else {
            ...
            for (int v_idx = tid; v_idx < tile_n * V_BLOCKS; v_idx += int(blockDim.x)) {
                ...
                const int k = k0 + kk;
                const int k_phys = dp16_fa_qblock_tail_page_physical_k(qblock_program, k);
                v_raw_q8_tile[kk * V_BLOCKS + blk] = k_phys >= 0 ? *pdmq_v_q8_0_block_ptr(
                    V, v_nb10, v_nb11, v_nb12, v_nb13, v_ne13, k_phys, hk, b, blk) : block_q8_0{};
            }
            __syncthreads();
        } else if constexpr (RAW_F16_LDS) {
            constexpr int HALF_PER_VEC = int(sizeof(int4) / sizeof(half));
            ...
            if (v_nb10 == int64_t(sizeof(half))) {
                for (int v_idx = tid; v_idx < tile_n * VECS_PER_ROW; v_idx += int(blockDim.x)) {
                    ...
                    const int k   = k0 + kk;
                    const int k_phys = dp16_fa_qblock_tail_page_physical_k(qblock_program, k);
                    const half * src = k_phys >= 0 ? pdmq_v_f16_ptr(V, v_nb10, v_nb11, v_nb12, v_nb13, v_ne13, k_phys, hk, b, d) : nullptr;
                    ((int4 *) (v_raw_f16_tile + kk * D))[vec] = src ? *((const int4 *) src) : int4{};
                }
            } else {
                for (int v_idx = tid; v_idx < tile_n * D; v_idx += int(blockDim.x)) {
                    const int kk = v_idx / D;
                    ...
                    const int k = k0 + kk;
                    const int k_phys = dp16_fa_qblock_tail_page_physical_k(qblock_program, k);
                    v_raw_f16_tile[kk * D + d] = k_phys >= 0 ? *pdmq_v_f16_ptr(
                        V, v_nb10, v_nb11, v_nb12, v_nb13, v_ne13, k_phys, hk, b, d) : __float2half(0.0f);
                }
            }
            __syncthreads();
        }
            ...
            for (int v_idx = tid; v_idx < BN * D; v_idx += int(blockDim.x)) {
                ...
                if (kk < tile_n) {
                    if constexpr (STAGE_V) {
                        ...
                        vv = pdmq_decode_v_q4_0_raw_lds<D>(v_raw_tile, kk, d);
                    } else if constexpr (RAW_Q8_LDS) {
                        vv = pdmq_decode_v_q8_0_raw_lds<D>(v_raw_q8_tile, kk, d);
                    } else if constexpr (RAW_F16_LDS) {
                        vv = pdmq_decode_v_f16_raw_lds<D>(v_raw_f16_tile, kk, d);
                    } else {
                        const int k = k0 + kk;
                        const int k_phys = dp16_fa_qblock_tail_page_physical_k(qblock_program, k);
                        vv = k_phys >= 0 ? pdmq_decode_v<V_TYPE>(V, v_nb10, v_nb11, v_nb12, v_nb13, v_ne13, k_phys, hk, b, d, V4_tail, v4_tail_nb10, v4_tail_nb11, v4_tail_nb12, v4_live_nk) : 0.0f;
                    }
                }
        ...
        if constexpr (KSHARED) {
            ...
            for (int idx = tid; idx < tile_n * KS_PER_ROW; idx += int(blockDim.x)) {
                ...
                const int k = k0 + kk;
                const int k_phys = dp16_fa_qblock_tail_page_physical_k(qblock_program, k);
                k_scales_s[kk * (KS_PER_ROW + 1) + s_val] = k_phys >= 0 ? k_scales[ggml_cuda_packed16_k_scale_index_from_desc(k_desc, (uint32_t) hk, (uint32_t) k_phys, (uint32_t) s_val)] : __float2half(0.0f);
            }
            __syncthreads();
        }
        ...
        for (int idx = tid; idx < GQA_GROUP * BM * BN; idx += int(blockDim.x)) {
            ...
            const int k_phys = valid ? dp16_fa_qblock_tail_page_physical_k(qblock_program, k) : k;
            valid = valid && k_phys >= 0 && k_phys < (int) k_kv_capacity;
            if (valid) {
                const int  * kp_row;
                const half * ks_row;
                if constexpr (KSHARED) {
                    ...
                    const size_t k_row = k_head_base + size_t(k_phys);
                    kp_row = k_payload + k_row * (D / 4);
                    ks_row = k_scales  + k_row * (D / QK8_0);
                }
                s_val = KSHARED
                    ? pdmq_qk_dot<BM, D>(q_tile_i32[slot], q_tile_scales[slot], qr, kp_row, ks_row) * attention_scale + mask_bias
                    : pdmq_qk_dot_sidecar<BM, D>(q_tile_i32[slot], q_tile_scales[slot], qr, k_payload, k_scales, k_desc, k_format, k_kv_capacity, hk, k_phys) * attention_scale + mask_bias;
            }
...
#pragma unroll
                for (int kk = 0; kk < BN; ++kk) {
                    const int k = k0 + kk;
                    const int k_phys = kk < tile_n ? dp16_fa_qblock_tail_page_physical_k(qblock_program, k) : -1;
                    const float vv = kk < tile_n && k_phys >= 0
                        ? (v4_144_pv4_active
                            ? (v4_144_pv4_words_active ? pdmq_decode_v_v4_k16d16_144_pv4_cached<D>(v4_144_pv4_payload_words, v4_144_scale_tile, kk, tid) : pdmq_decode_v_v4_k16d16_144_scale_lds<D>(V, v_nb11, v_nb12, v_nb13, v_ne13, k_phys, hk, b, tid, v4_144_scale_tile, kk))
                            : (RAW_Q4_LDS
                                ? pdmq_decode_v_q4_0_raw_or_k16d16<D, BN, V4_K16D16_ORACLE>(v_raw_tile, v4_k16d16_words, kk, tid)
                                : (RAW_Q8_LDS
                                    ? pdmq_decode_v_q8_0_raw_lds<D>(v_raw_q8_tile, kk, tid)
                                    : (RAW_F16_LDS
                                        ? pdmq_decode_v_f16_raw_lds<D>(v_raw_f16_tile, kk, tid)
                                        : pdmq_decode_v<V_TYPE>(V, v_nb10, v_nb11, v_nb12, v_nb13, v_ne13, k_phys, hk, b, tid, V4_tail, v4_tail_nb10, v4_tail_nb11, v4_tail_nb12, v4_live_nk)))))
                        : 0.0f;
#pragma unroll
                    for (int slot = 0; slot < GQA_GROUP; ++slot) {
#pragma unroll
                ...
                if (split_mode) {
                    ...
                    if (pdmq_debug_state_const && b == 0 && nk >= pdmq_debug_state_min_nk_const &&
                        ...
                        pdmq_scalar_ref_range<V_TYPE, BM, D, CAUSAL_MASK>(
                                ...
                                implicit_n_kv, implicit_q_offset, implicit_flags,
                                k_payload, k_scales,
                    ...
                    if (pdmq_mask_trace_const && tid == 0 && b == 0 && hq == 0 && q < 8) {
                        ...
                        for (int kk = 0; kk < nk; ++kk) {
                            ...
                            if (valid && (mask || (implicit_flags & 1))) {
                                valid = pdmq_mask_keep_and_bias(mask, mask_ne00, mask_ne01, mask_ne03, mask_nb00, mask_nb01, mask_nb03, q, kk, b, &bias_tmp, implicit_n_kv, implicit_q_offset, implicit_flags);
                            }
                            if (valid) {
                                ++valid_count;
                                last_valid = kk;
                        ...
                        const bool keep0 = (mask || (implicit_flags & 1)) ? pdmq_mask_keep_and_bias(mask, mask_ne00, mask_ne01, mask_ne03, mask_nb00, mask_nb01, mask_nb03, q, 0, b, &b0, implicit_n_kv, implicit_q_offset, implicit_flags) : true;
                        const bool keeplast = last_valid >= 0 && (mask || (implicit_flags & 1)) ? pdmq_mask_keep_and_bias(mask, mask_ne00, mask_ne01, mask_ne03, mask_nb00, mask_nb01, mask_nb03, q, last_valid, b, &blast, implicit_n_kv, implicit_q_offset, implicit_flags) : (last_valid >= 0);
                        const bool keepnk = (mask || (implicit_flags & 1)) ? pdmq_mask_keep_and_bias(mask, mask_ne00, mask_ne01, mask_ne03, mask_nb00, mask_nb01, mask_nb03, q, nk - 1, b, &bnk, implicit_n_kv, implicit_q_offset, implicit_flags) : true;
...
#pragma unroll
    ...
    for (int k0 = k_begin; k0 < k_end; k0 += BN) {
        ...
        for (int idx = tid; idx < GQA_GROUP * BN; idx += int(blockDim.x)) {
            ...
            float mask_bias = 0.0f;
            if (valid && (mask || (implicit_flags & 1))) {
                valid = pdmq_mask_keep_and_bias(mask, mask_ne00, mask_ne01, mask_ne03, mask_nb00, mask_nb01, mask_nb03, q0, k, b, &mask_bias, implicit_n_kv, implicit_q_offset, implicit_flags);
            }
            if (valid) {
...
#pragma unroll
    ...
    for (int k0 = 0; k0 < nk; k0 += BN) {
        ...
        if (tid < BM * BN) {
                ...
                float mask_bias = 0.0f;
                if (!exact_tile && valid && (mask || implicit_causal)) {
                    valid = pdmq_mask_keep_and_bias(mask, mask_ne00, mask_ne01, mask_ne03, mask_nb00, mask_nb01, mask_nb03, q, k, b, &mask_bias, implicit_n_kv, implicit_q_offset, implicit_flags);
                }
...
static inline bool pdmq_qblock_gqa4_m16_verify_candidate_enabled() {
    const char * env = getenv("GGML_CUDA_ROCM_MTP_QBLOCK_GQA4_M16_VERIFY_CANDIDATE");
...
void ggml_cuda_flash_attn_ext_packed16_dot4_mmq(
    ...
    const bool qwen9b_gqa4_topology =
        n_heads_q == 16 && n_heads_k == 4 && (n_heads_q / n_heads_k) == 4;
    ...
    if (pdmq_qblock_gqa4_m16_verify_candidate_enabled() && qblock_inst &&
            qwen9b_gqa4_topology && V->type == GGML_TYPE_V4_K16D16_144 && nq > 8 && nq <= 10) {
        // Cap-10 p2 shared-context verifier candidate: two slots with sampled+4
        // draft rows produce nq=10.  Keep nq<=8 on the validated M8N32 path;
        // only the wider combined verifier group gets M16N16.
    ...
    const int v4_144_qwen9b_gqa4_auto_min_nk = []() {
        const char * s = getenv("GGML_CUDA_ROCM_V4_K16D16_144_MTP_QBLOCK_GQA4_AUTO_MIN_NK");
        return s && *s ? atoi(s) : 2048;
    }();
    const bool v4_144_qwen9b_gqa4_auto =
        qblock_inst && v4_144_pv4_requested && !requested_gqa_group_env_set &&
        qwen9b_gqa4_topology && nq <= 8 && nk >= v4_144_qwen9b_gqa4_auto_min_nk && []() {
            const char * v = getenv("GGML_CUDA_ROCM_V4_K16D16_144_MTP_QBLOCK_GQA4_AUTO");
            return !v || atoi(v) != 0;
        }();
    ...
    if (v4_144_qwen9b_gqa4_auto) {
        requested_gqa_group = 4;
    } else if (v4_144_gqa6_wavegroup_requested && qwen27b_gqa6_topology && nq == 1) {
    ...
    } else if (v4_144_qwen9b_gqa4_auto && !gqax_splitk_env_set) {
    ...
    if (gqax_splitk_requested > 1 && nk < gqax_splitk_min_nk && !v4_decode_splitk_env_set && !v4_144_qwen9b_gqa4_auto) {
    ...
    const bool gqa4_m16_verify_candidate = pdmq_qblock_gqa4_m16_verify_candidate_enabled() && qblock_inst &&
        shape == PDMQ_SHAPE_M16N16 && V->type == GGML_TYPE_V4_K16D16_144 &&
        n_heads_q == 16 && n_heads_k == 4 && gqa_ratio == 4 && nq > 8 && nq <= 10;
    const bool gqa4_splitk_plan_supported = request_gqa4 && gqax_splitk_requested > 1 &&
        ((shape == PDMQ_SHAPE_M1N32 || shape == PDMQ_SHAPE_M2N32 || shape == PDMQ_SHAPE_M4N32 ||
          shape == PDMQ_SHAPE_M8N32) ? (nq <= 8) : gqa4_m16_verify_candidate) &&
    ...
    const bool gqa4_splitk_supported = is_gqa4 && !kshared &&
        ...
        n_heads_q == 16 && n_heads_k == 4 && gqa_ratio == 4 && (nq <= 8 || gqa4_m16_verify_candidate);
    ...
    if (qblock_program.enabled && batch != 1) {
        const char * unsafe_multi_batch = getenv("GGML_CUDA_ROCM_MTP_QBLOCK_UNSAFE_MULTI_BATCH");
        if (!unsafe_multi_batch || atoi(unsafe_multi_batch) == 0) {
            fprintf(stderr,
                    "MTP_QBLOCK_DISABLED: reason=batch_not_one batch=%d nq=%d nk=%d hq=%d hk=%d gqa_ratio=%d graph_inst=%d override=GGML_CUDA_ROCM_MTP_QBLOCK_UNSAFE_MULTI_BATCH\n",
                    batch, nq, nk, n_heads_q, n_heads_k, gqa_ratio, (int) fa_inst_i32);
            qblock_program = dp16_fa_qblock_program_disabled();
        }
    }
    ...
    ggml_cuda_mtp_qblock_tail_page_map_v1 qblock_tail_page_map = {};
    llama_kv_cache_get_mtp_qblock_tail_page_map(K ? K->data : nullptr, &qblock_tail_page_map);
    bool qblock_tail_page_consumer_bound = false;
    const char * qblock_tail_page_consumer_reason = "not_requested";
    if (qblock_txn_tail_page_consumer_req) {
        ...
        if (qblock_tail_page_map.active) {
            ...
            if (K->type != GGML_TYPE_I32 || !k_persistent_i32 || pdmq_k_format != GGML_CUDA_PDMQ_K_FORMAT_PACKED16_Q8_272) {
                ...
                qblock_tail_page_consumer_reason = "not_v4_144_direct_v";
            } else if (qblock_tail_page_map.page_tokens != MTP_V4_144_PAGE_TOKENS) {
                ...
                qblock_program.tail_page_valid_tail_tokens = (int) qblock_tail_page_map.valid_tail_tokens;
                qblock_program.tail_page_page_tokens = (int) qblock_tail_page_map.page_tokens;
                qblock_program.tail_page_physical_pages = (int) qblock_tail_page_map.physical_pages;
                qblock_program.tail_page_block_table_pages = (int) qblock_tail_page_map.block_table_pages;
                for (int i = 0; i < DP16_FA_QBLOCK_TAIL_PAGE_MAX_PAGES; ++i) {
                    qblock_program.tail_page_block_table[i] = qblock_tail_page_map.block_table[i];
                }
                qblock_tail_page_consumer_bound = true;
            }
        }
    }
    ...
    if (qblock_txn_tail_page_consumer_req && strcmp(qblock_tail_page_consumer_reason, "no_registered_map") == 0 && nk > 0) {
    ...
    if (qblock_txn_tail_page_proof && qblock_inst) {
        ...
        mtp_v4_144_tail_state_status txn_state_status = MTP_V4_144_TAIL_STATE_OK;
        bool txn_state_ok = false;
        uint32_t txn_state_valid_tail_tokens = 0;
        uint32_t txn_state_free_count = 0;
        uint32_t txn_state_first_page_valid_tokens = 0;
        ...
        if (K->type != GGML_TYPE_I32 || !k_persistent_i32) {
            ...
            if (!local_linear) {
                ...
                if (txn_tail_desc_static_ok) {
                    ...
                    if (txn_lineage_ok) {
                        // The runtime tail descriptor above uses cache-global physical page ids.
                        // The tiny page-state contract is transaction-local, so remap the single
                        // eligible tail page to local page slot 0 for metadata commit/free-list proof.
                        int32_t state_block_table[1] = { 0 };
                        mtp_v4_144_tail_page_desc_v1 state_tail_desc = tail_desc;
                        state_tail_desc.block_table = state_block_table;
                        state_tail_desc.physical_pages = 3;
                        mtp_v4_144_tail_page_state_v1 state = {};
                        txn_state_status = mtp_v4_144_tail_state_init(state, state_tail_desc.logical_base_token, state_tail_desc.physical_pages);
                        uint32_t state_page = 0;
                        if (txn_state_status == MTP_V4_144_TAIL_STATE_OK) {
                            txn_state_status = mtp_v4_144_tail_state_alloc_page(state, MTP_V4_144_TAIL_PAGE_OWNER_TXN, &state_page);
                        }
                        if (txn_state_status == MTP_V4_144_TAIL_STATE_OK) {
                            state_block_table[0] = (int32_t) state_page;
                            txn_state_status = mtp_v4_144_tail_state_commit_lineage(state, state_tail_desc, lineage);
                        }
                        txn_state_ok = txn_state_status == MTP_V4_144_TAIL_STATE_OK;
                        txn_state_valid_tail_tokens = state.valid_tail_tokens;
                        txn_state_free_count = state.free_count;
                        txn_state_first_page_valid_tokens = state_page < state.physical_pages ? state.page_valid_tokens[state_page] : 0u;
                    }
                }
                ...
                } else if (!txn_state_ok) {
                    txn_tail_reason = "page_state_rejected";
                } else {
        ...
        fprintf(stderr,
            "MTP_QBLOCK_TXN_TAIL_PAGE: node=%s layer=%d graph_inst=%d proof=%d requested=%d active=0 eligible=%d reason=%s desc_status=%u lineage_status=%u state_status=%u lineage_rows=%u accepted_len=%u new_valid_tail=%u final_state_slot=%u state_valid_tail=%u state_page_valid=%u state_free=%u nq=%d nk=%d hq=%d hk=%d gqa_ratio=%d qprog=%d rows=%d rowmask=0x%x vpath=%s V=%s K=%s k_format=%s capacity=%u tail_phys_page=%u v4_page_stride=%lld k_page_stride=%llu\n",
            pdmq_fattn_node_name(dst), pdmq_layer_index, fa_inst_i32,
            qblock_txn_tail_page_proof ? 1 : 0,
            qblock_txn_tail_page_enable ? 1 : 0,
            (txn_tail_desc_static_ok && txn_lineage_ok && txn_state_ok) ? 1 : 0,
            txn_tail_reason,
            (unsigned) txn_tail_status,
            (unsigned) txn_lineage_status,
            (unsigned) txn_state_status,
            txn_lineage_rows,
            txn_lineage_accepted_len,
            txn_lineage_new_valid_tail_tokens,
            (unsigned) txn_lineage_final_state_slot,
            txn_state_valid_tail_tokens,
            txn_state_first_page_valid_tokens,
            txn_state_free_count,
    ...
    const bool pdmq_dbv_paged_requested = getenv("GGML_CUDA_ROCM_PACKED16_DBV_PAGED_ATTENTION") &&
        atoi(getenv("GGML_CUDA_ROCM_PACKED16_DBV_PAGED_ATTENTION")) != 0;
    const bool pdmq_dbv_paged_trace = getenv("GGML_CUDA_ROCM_PACKED16_DBV_PAGED_ATTENTION_TRACE") &&
        atoi(getenv("GGML_CUDA_ROCM_PACKED16_DBV_PAGED_ATTENTION_TRACE")) != 0;
    if (pdmq_dbv_paged_requested && pdmq_dbv_paged_trace) {
        static std::atomic<int> pdmq_dbv_skip_log_count{0};
        const int skip_log_idx = pdmq_dbv_skip_log_count.fetch_add(1, std::memory_order_relaxed);
        if (skip_log_idx < 64) {
            fprintf(stderr,
                "PDMQ DBV_PAGED_ATTENTION_SKIP: reason=backend_not_pwmma_dbv route=rocm_packed16_dot4_mmq qblock=%d qprog=%d graph_inst=%d nq=%d nk=%d hq=%d hk=%d gqa_ratio=%d vpath=%s page_authority_env=%d tail_consumer_requested=%d tail_dispatch_bound=%d tail_reason=%s\n",
                qblock_inst ? 1 : 0,
                qblock_program.enabled ? 1 : 0,
                fa_inst_i32,
                nq,
                nk,
                n_heads_q,
                ...
                gqa_ratio,
                pdmq_v_path_name(plan.v_path),
                (getenv("GGML_CUDA_ROCM_MTP_QBLOCK_PAGE_AUTHORITY") && atoi(getenv("GGML_CUDA_ROCM_MTP_QBLOCK_PAGE_AUTHORITY")) != 0) ? 1 : 0,
                qblock_txn_tail_page_consumer_req ? 1 : 0,
                qblock_tail_page_dispatch_bound ? 1 : 0,
                qblock_tail_page_consumer_reason);
        }
...
#define PDMQ_QPACK_PROFILE_LOG(BRANCH) do { \
    if (dp16_qpack_stage_active && q_view.stage == DP16_FA_Q_STAGE_QPACK_I8_BLOCK32 && dp16_fa_qpack_profile_enabled()) { \
        static int s_pdmq_qpack_profile_count = 0; \
        const int qpack_profile_limit = dp16_fa_qpack_profile_limit(); \
        if (qpack_profile_limit < 0 || s_pdmq_qpack_profile_count++ < qpack_profile_limit) { \
            const unsigned long long qpack_prepared_rows = (unsigned long long) q_view.qpack_rows; \
            const unsigned long long qpack_row_visits = qpack_prepared_rows * (unsigned long long) (is_gqax_splitk ? gqax_splitk_active : 1); \
            const long long qpack_saved_inline_rows = qpack_row_visits >= qpack_prepared_rows ? \
                (long long) (qpack_row_visits - qpack_prepared_rows) : 0LL; \
            const double qpack_reuse_factor = qpack_prepared_rows > 0 ? \
                double(qpack_row_visits) / double(qpack_prepared_rows) : 0.0; \
            fprintf(stderr, \
                "PDMQ_QPACK_REUSE_PROFILE: branch=%s node=%s layer=%d graph_inst=%d role=%s variant=%s shape=%s " \
                "q_stage=%s qblock=%d qprog=%d qprec=%s qpack_layout=%d qpack_reuse_scope=%d " \
                "nq=%d nk=%d hq=%d hk=%d gqa_ratio=%d gqa_group=%d batch=%d split_k=%d split_k_raw=%d " \
                "k_shards_per_q_stage=%d k_blocks_total=%d qpack_rows=%llu row_visits=%llu " \
                "saved_inline_row_quant=%lld reuse_factor=%.2f qpack_payload_bytes=%zu qpack_scale_bytes=%zu " \
                "qpack_total_bytes=%zu qpack_prepare_ms=%.3f capture_skipped=%d grid=(%u,%u,%u)\n", \
                BRANCH, pdmq_fattn_node_name(dst), pdmq_layer_index, fa_inst_i32, \
                pdmq_role_name(plan.role), \
                is_gqa1_splitk ? "GQA1_SPLITK" : (is_gqa4_splitk ? "GQA4_SPLITK" : (is_gqa6_splitk ? "GQA6_SPLITK" : "GQAX_SPLITK")), \
                pdmq_shape_name(plan.shape), dp16_fa_q_stage_name(q_view.stage), qblock_inst ? 1 : 0, \
                qblock_program.enabled, dp16_fa_qblock_q_precision_mode_name(qblock_program.q_precision_mode), \
                qblock_program.qpack_layout, qblock_program.qpack_reuse_scope, \
                nq, nk, n_heads_q, n_heads_k, gqa_ratio, plan.gqa_group, batch, \
                is_gqax_splitk ? gqax_splitk_active : 1, gqax_splitk_active_raw, dp16_k_shards_per_q_stage, \
                gqax_k_blocks_total, qpack_prepared_rows, qpack_row_visits, qpack_saved_inline_rows, \
                qpack_reuse_factor, q_view.qpack_payload_bytes, q_view.qpack_scale_bytes, \
                q_view.qpack_payload_bytes + q_view.qpack_scale_bytes, q_view.qpack_prepare_ms, \
                q_view.qpack_profile_capture_skipped, grid.x, grid.y, grid.z); \
        } \
    } \
} while (0)
...
#undef PDMQ_LAUNCH_DEBUG_GQA1
        ...
        if (dp16_qpack_stage_active) {
            ...
            q_view.qblock_program = qblock_program;
            PDMQ_QPACK_PROFILE_LOG("gqa1_splitk");
        }
        ...
        if (shape != PDMQ_SHAPE_M1N32 && shape != PDMQ_SHAPE_M2N32 && shape != PDMQ_SHAPE_M4N32 &&
                shape != PDMQ_SHAPE_M8N32 && !(gqa4_m16_verify_candidate && shape == PDMQ_SHAPE_M16N16)) {
            GGML_ABORT("PDMQ QWEN35_DEBUG_ONLY GQA4 split-K supports only M1/M2/M4/M8N32 plus explicit M16 candidate, got shape=%s", pdmq_shape_name(shape));
        }
        const size_t partial_rows = size_t(gqax_splitk_active) * size_t(batch) * size_t(nq) * size_t(n_heads_q);
        ggml_cuda_pool & pool = ctx.pool();
        dp16_fa_q_workspace q_workspace(pool);
        if (dp16_qpack_stage_active) {
            q_view = dp16_fa_prepare_q_stage<PDMQ_D, PDMQ_THREADS>(stream, q_stage, Q, q_workspace);
            q_view.qblock_program = qblock_program;
            PDMQ_QPACK_PROFILE_LOG("gqa4_splitk");
        }
        ggml_cuda_pool_alloc<float> partial_m(pool, partial_rows);
        ggml_cuda_pool_alloc<float> partial_l(pool, partial_rows);
        ggml_cuda_pool_alloc<float> partial_out(pool, partial_rows * size_t(PDMQ_D));
        ...
        if (dp16_qpack_stage_active) {
            q_view = dp16_fa_prepare_q_stage<PDMQ_D, PDMQ_THREADS>(stream, q_stage, Q, q_workspace);
            q_view.qblock_program = qblock_program;
            PDMQ_QPACK_PROFILE_LOG("gqa6_splitk");
        }
        ggml_cuda_pool_alloc<float> partial_m(pool, partial_rows);
        ggml_cuda_pool_alloc<float> partial_l(pool, partial_rows);
        ggml_cuda_pool_alloc<float> partial_out(pool, partial_rows * size_t(PDMQ_D));
