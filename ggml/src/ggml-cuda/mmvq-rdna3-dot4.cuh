...
static inline bool ggml_cuda_rdna3_mmvq_dot4_supported(
    ...
    const bool base = ggml_cuda_rdna3_mmvq_dot4_env_enabled() && GGML_CUDA_CC_IS_RDNA3_0(cc) &&
           warp_size == GGML_CUDA_RDNA3_MMVQ_DOT4_WARP_SIZE && type == GGML_TYPE_Q4_K &&
           ncols_dst == 1 && (!has_ids || ggml_cuda_rdna3_mmvq_dot4_ids_enabled()) &&
           ncols_x % QK_K == 0 && nrows_x > 0;
...
static inline bool ggml_cuda_rdna3_mmvq_dot4_serial_columns_supported(
        const ggml_type type, const int cc, const int warp_size, const int ncols_dst,
        const bool has_fusion, const bool has_ids, const int ncols_x, const int nrows_x,
        const int32_t * q8sum4) {
#if defined(GGML_USE_HIP)
    ...
    const bool base = ggml_cuda_rdna3_mmvq_dot4_env_enabled() && GGML_CUDA_CC_IS_RDNA3_0(cc) &&
           warp_size == GGML_CUDA_RDNA3_MMVQ_DOT4_WARP_SIZE && type == GGML_TYPE_Q4_K &&
           ncols_dst > 1 && ncols_dst <= MMVQ_MAX_BATCH_SIZE && (!has_ids || ggml_cuda_rdna3_mmvq_dot4_ids_enabled()) &&
           ncols_x % QK_K == 0 && nrows_x > 0;
    return base && (!ggml_cuda_rdna3_mmvq_dot4_sideband_enabled() || q8sum4 != nullptr);
#else
    GGML_UNUSED_VARS(type, cc, warp_size, ncols_dst, has_fusion, has_ids, ncols_x, nrows_x, q8sum4);
...
// Stage4 verifier path: one HIP launch for several verifier columns while
// preserving the exact ncols_dst=1 RDNA3 dot4 arithmetic for each column.  The
// existing single-column kernel is called after the host offsets src1/dst; this
// variant flattens (sample,column) into blockIdx.z and applies those offsets
// inside the kernel, including ID lookup as ids[channel + column*ids_stride].
template <bool HAS_IDS, bool HAS_GATE, bool HAS_X_BIAS, bool HAS_GATE_BIAS, bool USE_Q8SUM4>
__launch_bounds__(GGML_CUDA_RDNA3_MMVQ_DOT4_ROWS_PER_BLOCK*GGML_CUDA_RDNA3_MMVQ_DOT4_WARPS_PER_ROW*GGML_CUDA_RDNA3_MMVQ_DOT4_WARP_SIZE, 1)
static __global__ void ggml_cuda_rdna3_mmvq_q4_k_dot4_serial_columns_kernel(
        const void * vx, const void * vy, const int32_t * y_q8sum4, const int32_t * ids,
        const ggml_cuda_mm_fusion_args_device fusion, float * dst, const uint32_t ncols_x, const uint32_t nrows_x,
        const uint3 nchannels_y, const uint3 channel_ratio, const uint3 sample_ratio, const uint32_t ncols_dst,
        const uint32_t stride_row_x, const uint32_t stride_col_y, const uint32_t stride_col_dst,
        const uint32_t stride_channel_x, const uint32_t stride_channel_y, const uint32_t stride_channel_dst,
        const uint32_t stride_sample_x, const uint32_t stride_sample_y, const uint32_t stride_sample_dst,
        const uint32_t ids_stride) {
    ...
    if (active) {
        if constexpr (HAS_IDS) {
            channel_x = (uint32_t) ids[channel_dst + column_dst*ids_stride];
            channel_y = fastmodulo(channel_dst, nchannels_y);
            channel_bias = channel_x;
        } else {
            channel_x = fastdiv(channel_dst, channel_ratio);
    ...
    const block_q8_1 * y = ((const block_q8_1 *) vy) +
        sample_dst*stride_sample_y + channel_y*stride_channel_y + column_dst*stride_col_y;
    const int32_t * ysum4 = nullptr;
    if constexpr (USE_Q8SUM4) {
        ysum4 = y_q8sum4 + 4*(sample_dst*stride_sample_y + channel_y*stride_channel_y + column_dst*stride_col_y);
    }
    ...
    dst[sample_dst*stride_sample_dst + channel_dst*stride_channel_dst + column_dst*stride_col_dst + row] = result;
}
...
template <bool HAS_IDS, bool HAS_GATE, bool HAS_X_BIAS, bool HAS_GATE_BIAS, bool USE_Q8SUM4>
static inline void ggml_cuda_rdna3_mmvq_dot4_serial_columns_launch_kernel(
        const void * vx, const void * vy, const int32_t * y_q8sum4, const int32_t * ids,
        const ggml_cuda_mm_fusion_args_device fusion, float * dst, const int ncols_x, const int nrows_x,
        const int ncols_dst, const uint3 nchannels_y, const uint3 channel_ratio, const uint3 sample_ratio,
        const int stride_row_x, const int stride_col_y, const int stride_col_dst, const int nchannels_dst,
        const int stride_channel_x, const int stride_channel_y, const int stride_channel_dst, const int nsamples_dst,
        const int stride_sample_x, const int stride_sample_y, const int stride_sample_dst, const int ids_stride,
        cudaStream_t stream) {
    ...
    ggml_cuda_rdna3_mmvq_q4_k_dot4_serial_columns_kernel<HAS_IDS, HAS_GATE, HAS_X_BIAS, HAS_GATE_BIAS, USE_Q8SUM4>
        <<<block_nums, block_dims, 0, stream>>>(vx, vy, y_q8sum4, ids, fusion, dst, ncols_x, nrows_x, nchannels_y,
            channel_ratio, sample_ratio, ncols_dst, stride_row_x, stride_col_y, stride_col_dst, stride_channel_x, stride_channel_y,
            stride_channel_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
}

template <bool HAS_IDS, bool USE_Q8SUM4>
static inline void ggml_cuda_rdna3_mmvq_dot4_serial_columns_launch_fusion(
        const void * vx, const void * vy, const int32_t * y_q8sum4, const int32_t * ids,
        const ggml_cuda_mm_fusion_args_device fusion, float * dst, const int ncols_x, const int nrows_x,
        const int ncols_dst, const uint3 nchannels_y, const uint3 channel_ratio, const uint3 sample_ratio,
        const int stride_row_x, const int stride_col_y, const int stride_col_dst, const int nchannels_dst,
    ...
    const bool has_x_bias = fusion.x_bias != nullptr;
    const bool has_gate_bias = has_gate && fusion.gate_bias != nullptr;
#define GGML_RDNA3_SC_LAUNCH(G, XB, GB) \
    ggml_cuda_rdna3_mmvq_dot4_serial_columns_launch_kernel<HAS_IDS, G, XB, GB, USE_Q8SUM4>(vx, vy, y_q8sum4, ids, fusion, dst, ncols_x, nrows_x, \
        ncols_dst, nchannels_y, channel_ratio, sample_ratio, stride_row_x, stride_col_y, stride_col_dst, nchannels_dst, stride_channel_x, \
        stride_channel_y, stride_channel_dst, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, stream)
    if (has_gate) {
        if (has_x_bias) {
            if (has_gate_bias) GGML_RDNA3_SC_LAUNCH(true, true, true); else GGML_RDNA3_SC_LAUNCH(true, true, false);
        } else {
            if (has_gate_bias) GGML_RDNA3_SC_LAUNCH(true, false, true); else GGML_RDNA3_SC_LAUNCH(true, false, false);
        }
    } else {
        if (has_x_bias) GGML_RDNA3_SC_LAUNCH(false, true, false); else GGML_RDNA3_SC_LAUNCH(false, false, false);
    }
#undef GGML_RDNA3_SC_LAUNCH
}

template <ggml_type type>
static inline void ggml_cuda_rdna3_mmvq_dot4_serial_columns_launch(
        const void * vx, const void * vy, const int32_t * y_q8sum4, const int32_t * ids,
        const ggml_cuda_mm_fusion_args_device fusion, float * dst, const int ncols_x, const int nrows_x,
        const int ncols_dst, const uint3 nchannels_y, const uint3 channel_ratio, const uint3 sample_ratio,
        const int stride_row_x, const int stride_col_y, const int stride_col_dst, const int nchannels_dst,
    ...
    if constexpr (type == GGML_TYPE_Q4_K) {
        ...
        if (ggml_cuda_rdna3_mmvq_dot4_log_enabled()) {
            GGML_LOG_INFO("%s: route=rdna3_mmvq_dot4_serial_columns mode=%s ids=%d rows=%d cols=%d ncols_dst=%d channels=%d samples=%d\n",
                __func__, use_sideband ? "q8sum4_sideband" : "exact_min_fallback", has_ids ? 1 : 0, nrows_x, ncols_x, ncols_dst, nchannels_dst, nsamples_dst);
        }
#define GGML_RDNA3_SC_TOP(IDS, SB) \
    ggml_cuda_rdna3_mmvq_dot4_serial_columns_launch_fusion<IDS, SB>(vx, vy, y_q8sum4, ids, fusion, dst, ncols_x, nrows_x, \
        ncols_dst, nchannels_y, channel_ratio, sample_ratio, stride_row_x, stride_col_y, stride_col_dst, nchannels_dst, \
        stride_channel_x, stride_channel_y, stride_channel_dst, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, stream)
        if (has_ids) { if (use_sideband) GGML_RDNA3_SC_TOP(true, true); else GGML_RDNA3_SC_TOP(true, false); }
        else         { if (use_sideband) GGML_RDNA3_SC_TOP(false, true); else GGML_RDNA3_SC_TOP(false, false); }
#undef GGML_RDNA3_SC_TOP
    } else {
        GGML_UNUSED_VARS(vx, vy, y_q8sum4, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, nchannels_y, channel_ratio, sample_ratio,
...
template <ggml_type type>
static inline void ggml_cuda_rdna3_mmvq_dot4_serial_columns_launch(const void * vx, const void * vy, const int32_t * y_q8sum4, const int32_t * ids,
        const ggml_cuda_mm_fusion_args_device fusion, float * dst, const int ncols_x, const int nrows_x, const int ncols_dst,
        const uint3 nchannels_y, const uint3 channel_ratio, const uint3 sample_ratio, const int stride_row_x, const int stride_col_y,
        const int stride_col_dst, const int nchannels_dst, const int stride_channel_x, const int stride_channel_y,
        const int stride_channel_dst, const int nsamples_dst, const int stride_sample_x, const int stride_sample_y,
