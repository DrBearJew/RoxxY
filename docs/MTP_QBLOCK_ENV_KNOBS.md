...
| `GGML_CUDA_ROCM_MTP_QBLOCK_TXN_TAIL_PAGE_PROOF` | `0` | diagnostic/probe | packed16/V4_144 FA backend | Emits `MTP_QBLOCK_TXN_TAIL_PAGE` from the packed16 DOT4/PDMQ QBlock route. It performs static fail-closed eligibility checks for the transactional packed16-K + V4_144 tail-page ABI and reports descriptor status, but hard-codes `active=0`; it does not alter dispatch or commit state. |
| `GGML_CUDA_ROCM_MTP_QBLOCK_TXN_TAIL_PAGE` | `0` | disabled prototype gate | packed16/V4_144 FA backend | Requests the future transactional tail-page runtime path. Current implementation remains fail-closed and logs `reason=state_sampler_not_wired` when the FA-side descriptor is eligible; no production mutation or route change occurs until sampler/recurrent commit wiring is added and hash gates pass. |
