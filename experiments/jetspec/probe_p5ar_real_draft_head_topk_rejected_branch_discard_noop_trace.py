#!/usr/bin/env python3
"""Fast no-model P5AR real draft-head top-k rejected-branch discard no-op ABI trace probe."""
...
FORBIDDEN_TRACE_TOKENS = [
    ...
    "reuse_p5aq_hidden_kv_commit_noop=0",
...
SELF_TEST_TRACE = (
    "common_speculative_impl_draft_jetspec::process: draft-jetspec p5ar_real_draft_head_topk_rejected_branch_discard_noop_runtime "
    "phase=real_draft_head_topk_rejected_branch_discard_noop_ready real_topk_rejected_branch_discard_noop_runtime_ready=1 "
    "real_topk_rejected_branch_discard_noop_runtime_hash=fefefefefefefefe real_topk_hidden_kv_commit_noop_runtime_ready=1 "
    ...
    "reuse_p5aq_hidden_kv_commit_noop=1 rejected_branch_discard_noop=1 "
    ...
    "no_real_rejected_branch_discard=1 no_rejected_branch_discard=1 no_publish=1 no_visible_state_change=1 "
...
def validate_trace_line(line: str) -> dict[str, Any]:
    ...
    errors.extend(f"missing P5AR trace token: {token}" for token in missing)
    errors.extend(f"forbidden P5AR trace token present: {token}" for token in forbidden)

    scalar_keys = [
        "real_topk_rejected_branch_discard_noop_runtime_ready", "real_topk_hidden_kv_commit_noop_runtime_ready",
        "real_topk_token_commit_noop_runtime_ready", "real_topk_accept_path_descriptor_runtime_ready",
        "real_topk_accept_boundary_runtime_ready", "real_topk_verify_mask_runtime_ready",
        "real_topk_tree_runtime_ready", "real_topk_candidate_runtime_ready",
        ...
        "token_commit_noop", "hidden_kv_commit_noop", "reuse_p5aq_hidden_kv_commit_noop",
        "rejected_branch_discard_noop", "actual_committed_tokens", "actual_survivor_pages_committed",
        ...
        "no_real_rejected_branch_discard", "no_rejected_branch_discard", "no_publish", "no_visible_state_change",
    ...
    for key, value in expected_exact.items():
        ...
        if len(values) != 1:
            errors.append(f"P5AR trace {key} must appear exactly once with value {value}, got {values}")
        if not values or any(observed != value for observed in values):
            errors.append(f"P5AR trace {key} must be {value}")
    ...
    if candidate_ids is None or len(candidate_ids) != 2 or candidate_ids[0] < 0 or candidate_ids[1] < 0 or candidate_ids[0] == candidate_ids[1]:
        errors.append("P5AR trace candidate_ids must contain two distinct non-negative ids")
    if tree_ids is None or len(tree_ids) != 3:
        errors.append("P5AR trace real_tree_token_ids must contain root plus two candidates")
    elif candidate_ids is not None and tree_ids[1:] != candidate_ids:
        errors.append("P5AR trace real_tree_token_ids[1:] must equal P5AK candidate_ids")
    ...
    for path in [DOC, README, CANDIDATE]:
        ...
        for token in [
            ...
            if token not in text:
                errors.append(f"{path.name} missing P5AR contract token: {token}")
...
def _trace_from_log(path: pathlib.Path) -> str | None:
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        if "draft-jetspec p5ar_real_draft_head_topk_rejected_branch_discard_noop_runtime" in line:
            return line
    return None


def probe_p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace(trace_log: pathlib.Path | None = None) -> dict[str, Any]:
    errors: list[str] = []
    ...
    if trace_log is not None:
        if not trace_log.exists():
            ...
            if line is None:
                errors.append("trace log does not contain draft-jetspec p5ar_real_draft_head_topk_rejected_branch_discard_noop_runtime")
            else:
                live_runtime = True
    ...
    return {
        "ok": not errors,
        "status": "p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace_contract_verified" if not errors else "p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace_contract_invalid",
        "errors": errors,
        ...
        "limitations": [
            ...
            "P5AR reuses P5AQ hidden/KV commit no-op metadata to describe a rejected-branch discard no-op",
            "does not commit tokens, publish visible tokens, mutate hidden/KV, discard rejected branches, publish visible state, or emit draft tokens",
        ],
    }
...
def main(argv: list[str]) -> int:
    ...
    out = probe_p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace(args.trace_log)
