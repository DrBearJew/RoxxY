...
Status: candidate no-model contract wiring for the next post-P5AR real draft-head tail ABI slice.

Gate: `LLAMA_JETSPEC_REAL_DRAFT_HEAD_TOPK_PUBLISH_GATE_NOOP_ABI_ONLY=1`.

Trace contract:
- `draft-jetspec p5as_real_draft_head_topk_publish_gate_noop_runtime`
- `phase=real_draft_head_topk_publish_gate_noop_ready`
- `invalid_real_draft_head_topk_publish_gate_noop_runtime` on failed prerequisites
...
Required gate chain: P5AS requires P5AR plus P5AQ, P5AP, P5AO, P5AN, P5AM, P5AL, P5AK, P5AJ, P5AG, P5AF, P5AE, and P5X readiness.
...
- not the generic P5W publish-gate descriptor
- not the root P5AD publish-gate no-op path
- terminal real draft-head shadow tail slice, still no publish
- rejects `publish_gate_descriptor_ready=1`
- rejects `publish_runtime_ready=1`
...
This slice performs no target logits walk, no target accept walk, no accept, no real token commit, no visible token publish, no real hidden/KV commit, no hidden/KV commit, no real rejected-branch discard, no rejected-branch discard, no real publish, no publish, no visible state change, no KV mutation, and no draft tokens.
