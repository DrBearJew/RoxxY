#!/usr/bin/env python3
"""Validate the inert P5AU target-logits walk readiness packet."""
...
REQUIRED_FILES = [
    pathlib.Path("experiments/jetspec/jetspec_p5au_target_logits_walk_readiness.md"),
    pathlib.Path("experiments/jetspec/jetspec_p5au_target_logits_walk_readiness.py"),
    pathlib.Path("experiments/jetspec/validate_p5au_target_logits_walk_readiness.py"),
    pathlib.Path("experiments/jetspec/test_p5au_target_logits_walk_readiness.py"),
    pathlib.Path("experiments/jetspec/fixtures/jetspec_p5au_target_logits_walk_readiness_smoke.json"),
    pathlib.Path("experiments/jetspec/fixtures/jetspec_p5au_target_logits_walk_readiness_smoke.out.json"),
]
...
REQUIRED_TOKENS: dict[pathlib.Path, list[str]] = {
    pathlib.Path("experiments/jetspec/jetspec_p5au_target_logits_walk_readiness.md"): [
        ...
        "no KV mutation",
        "no draft tokens",
        "default-off target-logits walk canary",
    ],
    pathlib.Path("experiments/jetspec/jetspec_p5au_target_logits_walk_readiness.py"): [
        "p5au_target_logits_walk_readiness_verified_not_executed",
        "P5AT_STATUS",
        ...
        "EXPECTED_ALLOWED_EDGES",
        "parent_position_scores_candidate_children",
        "target_model_full_vocab_logits",
        "target_logits_walk_blocked_pending_explicit_approval",
    ],
    pathlib.Path("experiments/jetspec/test_p5au_target_logits_walk_readiness.py"): [
        "P5AUTargetLogitsWalkReadinessTests",
        "test_smoke_fixture_matches_expected_output",
        "test_rejects_missing_p5at_predecessor",
        "test_rejects_candidate_tree_mismatch",
        "test_rejects_wrong_planned_target_rows",
        "test_rejects_runtime_hook_and_approval",
        "test_rejects_actual_target_walk_or_accept",
...
AGGREGATE_TOKENS = [
    ...
    "jetspec_p5au_target_logits_walk_readiness.py",
    "validate_p5au_target_logits_walk_readiness.py",
    "p5au_target_logits_walk_readiness_verified_not_executed",
    "jetspec_p5au_target_logits_walk_readiness_smoke.json",
]
...
def _scan_forbidden_roots() -> list[dict[str, Any]]:
    ...
    for root in FORBIDDEN_ROOTS:
        ...
        for path in base.rglob("*"):
            ...
            matched = [token for token in P5AU_TOKENS if token in text]
            if matched:
                hits.append({"path": str(rel), "tokens": matched})
    return hits


def validate_p5au_target_logits_walk_readiness() -> dict[str, Any]:
    errors: list[str] = []

    for rel in REQUIRED_FILES:
        if not (REPO_ROOT / rel).exists():
            errors.append(f"missing required P5AU file: {rel}")
    ...
    for token in FORBIDDEN_CONTRACT_TOKENS:
        if token in contract_text:
            errors.append(f"P5AU no-runtime contract must not contain runtime side-effect token: {token}")
    ...
    try:
        ...
        if actual != expected:
            errors.append("P5AU smoke output differs from expected fixture output")
        if actual.get("status") != readiness.STATUS:
            errors.append(f"P5AU smoke status mismatch: {actual.get('status')}")
        plan = actual.get("target_logits_plan", {})
        if plan.get("planned_target_logits_rows") != 1:
            errors.append("P5AU must plan one future target logits row")
        if plan.get("actual_target_logits_rows_walked") != 0:
            errors.append("P5AU must not walk target logits rows")
        if actual.get("promotion_result") != "target_logits_walk_blocked_pending_explicit_approval":
            errors.append("P5AU must block target logits walk pending explicit approval")
        for key, value in actual.get("approvals", {}).items():
            if value is not False:
                errors.append(f"P5AU approval flag unexpectedly true: {key}")
        for key, value in actual.get("runtime_hooks", {}).items():
            if value is not False:
                errors.append(f"P5AU runtime hook unexpectedly true: {key}")
        if actual.get("runtime_executed") is not False or actual.get("target_logits_walk_executed") is not False:
            errors.append("P5AU must not execute runtime or target logits walk")
    except (OSError, json.JSONDecodeError, KeyError, ValueError) as exc:
        errors.append(f"P5AU smoke fixture failed: {exc}")

    forbidden_hits = _scan_forbidden_roots()
    for hit in forbidden_hits:
        errors.append(f"P5AU token appears in forbidden production path: {hit}")
    ...
    for rel in _cmake_files():
        ...
        if matched:
            ...
            errors.append(f"P5AU must not add explicit CMake wiring in {rel}: {matched}")

    return {
        "ok": not errors,
        "errors": errors,
        "status": "p5au_target_logits_walk_readiness_validated" if not errors else "p5au_target_logits_walk_readiness_invalid",
...
def main(argv: list[str]) -> int:
    parse_args(argv)
    out = validate_p5au_target_logits_walk_readiness()
