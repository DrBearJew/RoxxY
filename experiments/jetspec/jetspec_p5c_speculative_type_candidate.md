...
No `include/llama.h`, `tools/server/`, repository `tests/`, `examples/`, `pocs/`,
`ggml/src/`, production CMake wiring, or non-P5A/P5B model registration edits are
approved by P5C.
...
- Missing target/draft context bindings disables JetSpec before runtime execution.
...
- no `draft-jetspec` route appears in `include/llama.h`, `tools/server/`, CMake,
...
state, verify masks, hidden/KV rollback, draft-head graph execution, server
request behavior, and performance gates.
