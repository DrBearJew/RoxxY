#!/usr/bin/env python3
"""Fast no-model P5AL real draft-head top-k tree ABI trace probe."""
...
REQUIRED_TRACE_TOKENS = [
    "draft-jetspec p5al_real_draft_head_topk_tree_runtime",
    "phase=real_draft_head_topk_tree_ready",
...
SELF_TEST_TRACE = (
    "common_speculative_impl_draft_jetspec::process: draft-jetspec p5al_real_draft_head_topk_tree_runtime "
    "phase=real_draft_head_topk_tree_ready real_topk_tree_runtime_ready=1 "
    ...
    "correction_token_present=0 no_synthetic_token_ids=1 no_external_logits_walk=1 no_target_logits_walk=1 "
...
def validate_trace_line(line: str) -> dict[str, Any]:
    ...
    errors.extend(f"missing P5AL trace token: {token}" for token in missing)
    errors.extend(f"forbidden P5AL trace token present: {token}" for token in forbidden)
    ...
    for key, value in expected_exact.items():
        if parsed[key] != value:
            errors.append(f"P5AL trace {key} must be {value}")
    ...
    if candidate_ids is None:
        errors.append("P5AL trace candidate_ids must be present")
    elif len(candidate_ids) != 2 or candidate_ids[0] < 0 or candidate_ids[1] < 0 or candidate_ids[0] == candidate_ids[1]:
        errors.append("P5AL trace candidate_ids must contain two distinct non-negative ids")
    if tree_ids is None:
        errors.append("P5AL trace real_tree_token_ids must be present")
    elif len(tree_ids) != 3:
        errors.append("P5AL trace real_tree_token_ids must contain root plus two candidates")
    elif candidate_ids is not None and tree_ids[1:] != candidate_ids:
        errors.append("P5AL trace real_tree_token_ids[1:] must equal P5AK candidate_ids")
    if parent_indices != [-1, 0, 0]:
        errors.append("P5AL trace real_tree_parent_indices must be [-1,0,0]")
    if tree_depth != [0, 1, 1]:
        errors.append("P5AL trace real_tree_depth must be [0,1,1]")
    if tree_rank != [-1, 0, 1]:
        errors.append("P5AL trace real_tree_rank must be [-1,0,1]")
    if candidate_logits is None or len(candidate_logits) != 2 or candidate_logits[0] < candidate_logits[1]:
        errors.append("P5AL trace candidate_logits must be descending rank logits")
    if real_tree_logits is None or len(real_tree_logits) != 3:
        errors.append("P5AL trace real_tree_logits must include root plus candidate logits")
    elif candidate_logits is not None and real_tree_logits[1:] != candidate_logits:
        errors.append("P5AL trace real_tree_logits[1:] must equal candidate_logits")
...
def _source_contract() -> dict[str, Any]:
    ...
    errors: list[str] = []
    for token in [
        'common_speculative_env_enabled("LLAMA_JETSPEC_REAL_DRAFT_HEAD_TOPK_TREE_ABI_ONLY")',
        ...
        if token not in text:
            errors.append(f"missing P5AL source token: {token}")
    builder = _function_slice(text, "build_real_draft_head_topk_tree_runtime")
    if not builder:
        ...
        for token in ["llama_decode", "llama_graph", "llama_kv_cache", "result->push_back", "common_sampler_sample", "llama_sampler", "tree_accept"]:
            if token in builder:
                errors.append(f"forbidden P5AL builder token present: {token}")
        for token in [
            "real_draft_head_topk_tree_token_ids[1] = real_draft_head_topk_candidate_ids[0]",
            "real_draft_head_topk_tree_token_ids[2] = real_draft_head_topk_candidate_ids[1]",
            "real_draft_head_topk_tree_cum_logit[1] = real_draft_head_topk_candidate_logits[0]",
            ...
            if token not in builder:
                errors.append(f"P5AL builder missing real-tree assignment: {token}")
...
def _trace_from_log(path: pathlib.Path) -> str | None:
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        if "draft-jetspec p5al_real_draft_head_topk_tree_runtime" in line:
            return line
    return None


def probe_p5al_real_draft_head_topk_tree_trace(trace_log: pathlib.Path | None = None) -> dict[str, Any]:
    errors: list[str] = []
    ...
    if trace_log is not None:
        if not trace_log.exists():
            ...
            if line is None:
                errors.append("trace log does not contain draft-jetspec p5al_real_draft_head_topk_tree_runtime")
            else:
                live_runtime = True
    ...
    return {
        "ok": not errors,
        "status": "p5al_real_draft_head_topk_tree_trace_contract_verified" if not errors else "p5al_real_draft_head_topk_tree_trace_contract_invalid",
        "errors": errors,
        ...
        "limitations": [
            ...
            "P5AL records a shadow real-tree ABI; it does not rewrite P5AE/P5AF/P5AG synthetic hashes",
...
def main(argv: list[str]) -> int:
    ...
    out = probe_p5al_real_draft_head_topk_tree_trace(args.trace_log)
