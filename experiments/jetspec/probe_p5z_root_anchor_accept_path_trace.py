#!/usr/bin/env python3
"""Fast no-model P5Z root-anchor accept-path trace contract probe.

Default mode validates source plus a representative P5Z trace without loading a
...
REQUIRED_TRACE_TOKENS = [
    "draft-jetspec p5z_root_anchor_accept_path_runtime",
    "phase=accept_path_runtime_ready",
...
SELF_TEST_TRACE = (
    "common_speculative_impl_draft_jetspec::process: draft-jetspec p5z_root_anchor_accept_path_runtime "
    "phase=accept_path_runtime_ready root_anchor_accept_path_runtime_ready=1 "
    ...
    "correction_token_present=0 no_target_logits_walk=1 no_target_accept_walk=1 "
    "no_token_commit=1 no_hidden_kv_commit=1 no_rejected_branch_discard=1 "
    "no_publish=1 no_visible_state_change=1 no_kv_mutation=1 no_draft_head_graph=1 "
    "no_draft_tokens=1"
)


class P5ZTraceProbeError(RuntimeError):
    """Raised when a probe input cannot be inspected."""


def _source() -> str:
    if not SOURCE.exists():
        raise P5ZTraceProbeError(f"missing source: {SOURCE}")
...
def _slice(text: str, start_token: str, end_token: str) -> str:
    ...
    if start < 0 or end < 0 or end <= start:
        raise P5ZTraceProbeError(f"cannot isolate slice {start_token!r}..{end_token!r}")
...
def validate_trace_line(line: str) -> dict[str, Any]:
    ...
    errors.extend(f"missing P5Z trace token: {token}" for token in missing)
    errors.extend(f"forbidden P5Z trace token present: {token}" for token in forbidden)
    ...
    if root_anchor != 1:
        errors.append("P5Z trace root_verified_anchor must be 1")
    if path_len != 0:
        errors.append("P5Z trace accept_path_len must be 0")
    if actual_nodes != 1:
        errors.append("P5Z trace actual_tree_nodes must be exactly 1")
    if mask_entries != 1:
        errors.append("P5Z trace actual_verify_mask_entries must be exactly 1")
    if accepted != 0:
        errors.append("P5Z trace actual_accepted_nodes must stay 0")
    if correction != 0:
        errors.append("P5Z trace correction_token_present must stay 0")
...
def _source_contract() -> dict[str, Any]:
    ...
    build = _slice(impl, "bool build_root_anchor_accept_path_runtime()", "bool build_accept_path_descriptor()")

    errors: list[str] = []
    required_source_tokens = [
        'common_speculative_env_enabled("LLAMA_JETSPEC_ROOT_ANCHOR_ACCEPT_PATH_ONLY")',
    ...
    for token in required_source_tokens:
        if not any(token in haystack for haystack in haystacks):
            errors.append(f"missing P5Z source trace/negative token: {token}")

    forbidden_after_anchor = [
    ...
    for token in forbidden_after_anchor:
        if token in branch:
...
def _trace_from_log(path: pathlib.Path) -> str | None:
    ...
    for line in text.splitlines():
        if "draft-jetspec p5z_root_anchor_accept_path_runtime" in line:
            return line
    return None


def probe_p5z_root_anchor_accept_path_trace(trace_log: pathlib.Path | None = None) -> dict[str, Any]:
    errors: list[str] = []
    ...
    if trace_log is not None:
        if not trace_log.exists():
            ...
            if line is None:
                errors.append("trace log does not contain draft-jetspec p5z_root_anchor_accept_path_runtime")
            else:
    ...
    return {
        "ok": not errors,
        "status": "p5z_root_anchor_accept_path_trace_contract_verified" if not errors else "p5z_root_anchor_accept_path_trace_contract_invalid",
        "errors": errors,
        ...
        "limitations": [
            ...
            "does not execute draft-head graph, token commit, KV mutation, publish, or draft tokens",
        ],
    }
...
def main(argv: list[str]) -> int:
    ...
    try:
        out = probe_p5z_root_anchor_accept_path_trace(args.trace_log)
    except P5ZTraceProbeError as exc:
        out = {"ok": False, "status": "p5z_root_anchor_accept_path_trace_probe_error", "errors": [str(exc)]}
    if args.json or not out.get("ok"):
        print(json.dumps(out, indent=2, sort_keys=True))
    else:
        print(f"P5Z trace contract probe passed status={out['status']} runtime_executed={str(out['runtime_executed']).lower()}")
