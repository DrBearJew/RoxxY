...
preflight, P5N transaction-plan scaffold, P5O pre-round snapshot descriptor, P5P transient-reservation descriptor, and P5Q tree-build descriptor have explicit approval as bounded
production candidates; executable JetSpec tree drafting remains blocked until
its own approval.
...
P5F artifact status: `validate_p5f_artifact_binding.py` now verifies the stored real JetSpec draft-head manifest/tensor map/conversion plan against P5F source constants and reports `artifact_verified_not_runtime_executed`. This covers 91 BF16 tensors, `fc.weight` shape `[2048, 10240]`, target tap layers `[1, 10, 19, 28, 37]`, target hidden size 2048, tap width 10240, draft block size 16, 8 draft layers, 32 attention heads, 4 KV heads, vocab size 248320, `requires_target_embeddings=true`, `requires_target_lm_head=true`, and `runtime_supported=false`. `probe_p5f_loader_gate.py` then writes the metadata-only preview and verifies the built `llama-cli` fails closed as `preview_not_allowed` by default and `unsupported_runtime` with `LLAMA_JETSPEC_ALLOW_PREVIEW_LOAD=1`, reporting `loader_gate_verified_preflight_still_blocked`. It still does not instantiate a target/draft `llama_context` pair, execute `common_speculative_jetspec_preflight`, execute the draft-head graph, or build/verify a tree.
...
- `validate_p5c_speculative_type.py` verifies `COMMON_SPECULATIVE_TYPE_DRAFT_JETSPEC`, `draft-jetspec` parsing, the `jetspec` alias, the `runtime_supported=false` no-draft placeholder, no silent fallback to `draft-simple`, no public/server/CMake route, and no kernel/test/example/poc wiring.
...
- `jetspec_p5f_binding_preflight_candidate.md` records the approved P5F hook set, target/draft context/model/vocab checks, metadata/shape checks, Qwen3.6 shape constants, target tap count/width checks, fail-closed disable path, no-draft behavior, and blocked tree-runtime work.
- `validate_p5f_binding_preflight.py` verifies binding preflight, metadata/shape checks, target tap count/width checks, no draft tokens, no draft-head graph execution, and no public/server/CMake/kernel/test/example/poc wiring.
...
- `probe_p5f_loader_gate.py` verifies the actual built loader rejects the metadata-only preview as `preview_not_allowed` by default and `unsupported_runtime` with `LLAMA_JETSPEC_ALLOW_PREVIEW_LOAD=1`, reporting `loader_gate_verified_preflight_still_blocked` while keeping P5F live preflight blocked.
...
| non-P5C/P5D/P5E/P5F/P5N/P5O/P5P/P5Q `common/` files | forbidden | next runtime approval |
| `tools/server/` | forbidden | runtime approval |
