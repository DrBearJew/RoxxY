#!/usr/bin/env python3
"""Validate the P5T JetSpec token-commit descriptor source slice."""
...
REQUIRED_TOKENS: dict[pathlib.Path, list[str]] = {
    pathlib.Path("common/speculative.cpp"): ['JETSPEC_TOKEN_COMMIT_PHASE', 'JETSPEC_TOKEN_COMMIT_ROLLBACK_POINT', 'JETSPEC_TOKEN_COMMIT_DESCRIPTOR', 'commit_tokens', 'after_token_commit', 'token_commit_descriptor_only', 'token_commit_descriptor_ready', 'invalid_token_commit_descriptor', 'n_token_commit_descriptors', 'token_commit_descriptor_hash_last', 'build_token_commit_descriptor', '!accept_path_descriptor_ready || accept_path_descriptor_hash_last == 0', 'actual_committed_tokens_last = 0', 'common_speculative_fnv1a64(JETSPEC_TOKEN_COMMIT_PHASE', 'common_speculative_fnv1a64(JETSPEC_TOKEN_COMMIT_ROLLBACK_POINT', 'disable_runtime_state(jetspec_runtime_failure::invalid_token_commit_descriptor', 'token_commit_descriptor_ready=%d', 'token_commit_descriptor_hash=%016', 'commit_tokens', 'actual_committed_tokens=0', 'token_commit_descriptor_only=1', 'no_real_token_commit=1', 'no_visible_token_publish=1', 'no_kv_mutation=1', 'no_publish=1', 'no_draft_tokens=1', 'token_commit_phase=commit_tokens', 'rollback_point=after_token_commit'],
    pathlib.Path("docs/speculative.md"): ['P5T token-commit descriptor', 'commit_tokens', 'token-commit descriptor', 'actual_committed_tokens=0', 'no real token commit', 'no visible token publish', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft tokens'],
    pathlib.Path("experiments/jetspec/jetspec_p5t_token_commit_descriptor_candidate.md"): ['JetSpec P5T token-commit descriptor candidate', 'approved bounded production-source slice', 'default-off and non-drafting', 'common/speculative.cpp', 'docs/speculative.md', 'token_commit_descriptor_ready', 'invalid_token_commit_descriptor', 'actual_committed_tokens=0', 'no real token commit', 'no visible token publish', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft-head graph execution', 'no draft tokens emitted', 'no real KV mutation', 'no CUDA dispatch', 'no server route', 'no public API', 'no CMake wiring'],
}

P5_TOKENS = ['P5T token-commit descriptor', 'jetspec_p5t_token_commit_descriptor', 'validate_p5t_token_commit_descriptor', 'test_p5t_token_commit_descriptor', 'JETSPEC_TOKEN_COMMIT_PHASE', 'JETSPEC_TOKEN_COMMIT_ROLLBACK_POINT', 'JETSPEC_TOKEN_COMMIT_DESCRIPTOR', 'token_commit_descriptor_ready', 'invalid_token_commit_descriptor', 'token_commit_descriptor_hash_last']
...
def validate_p5t_token_commit_descriptor() -> dict[str, Any]:
    errors: list[str] = []
    ...
    if "build_token_commit_descriptor()" not in impl:
        errors.append("descriptor builder is missing")
    for token in ['actual_committed_tokens_last']:
        if f"{token} = 0" not in impl:
            errors.append(f"descriptor must keep {token} at zero")
    if "disable_runtime_state(jetspec_runtime_failure::invalid_token_commit_descriptor" not in impl:
    ...
    for token in ['actual_committed_tokens=0', 'no real token commit', 'no visible token publish', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft tokens']:
    ...
    return {
        "ok": not errors,
        "errors": errors,
        "status": "p5t_token_commit_descriptor_validated" if not errors else "p5t_token_commit_descriptor_invalid",
...
def main(argv: list[str]) -> int:
    parse_args(argv)
    out = validate_p5t_token_commit_descriptor()
