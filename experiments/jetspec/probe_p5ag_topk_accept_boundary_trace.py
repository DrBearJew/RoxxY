#!/usr/bin/env python3
"""Fast no-model P5AG top-k accept-boundary ABI trace contract probe."""
...
REQUIRED_TRACE_TOKENS = [
    "draft-jetspec p5ag_topk_accept_boundary_runtime",
...
SELF_TEST_TRACE = (
    "common_speculative_impl_draft_jetspec::process: draft-jetspec p5ag_topk_accept_boundary_runtime "
    "phase=accept_path_runtime_ready topk_accept_boundary_runtime_ready=1 "
    ...
    "topk_tree_runtime_hash=aaaaaaaaaaaaaaaa actual_tree_nodes=3 actual_verify_mask_entries=5 "
    "accept_boundary_candidate_nodes=2 accept_boundary_verified_edges=5 actual_verified_logits_rows=0 "
    "accept_decision_source=none_no_logits accept_path_len=0 actual_accepted_nodes=0 correction_token_present=0 "
    "no_target_logits_walk=1 no_target_accept_walk=1 no_token_commit=1 no_hidden_kv_commit=1 "
    "no_rejected_branch_discard=1 no_publish=1 no_visible_state_change=1 no_kv_mutation=1 "
    "no_draft_head_graph=1 no_draft_tokens=1"
)
...
def validate_trace_line(line: str) -> dict[str, Any]:
    ...
    errors.extend(f"missing P5AG trace token: {token}" for token in missing)
    errors.extend(f"forbidden P5AG trace token present: {token}" for token in forbidden)
    expected = {
        "actual_tree_nodes": 3,
        "actual_verify_mask_entries": 5,
        "accept_boundary_candidate_nodes": 2,
    ...
    for key, value in expected.items():
        if parsed[key] != value:
            errors.append(f"P5AG trace {key} must be {value}")
...
def _source_contract() -> dict[str, Any]:
    ...
    errors: list[str] = []
    for token in [
        'common_speculative_env_enabled("LLAMA_JETSPEC_ACCEPT_PATH_TOPK_ABI_ONLY")',
        ...
        if token not in text:
            errors.append(f"missing P5AG source token: {token}")
    branch_start = text.find("bool build_topk_accept_boundary_runtime()")
    ...
    for token in ["llama_decode", "llama_kv_cache", "result->push_back", "tree_accept", "common_sampler_sample", "llama_sampler"]:
        if token in branch:
            errors.append(f"forbidden P5AG source token present: {token}")
...
def _trace_from_log(path: pathlib.Path) -> str | None:
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        if "draft-jetspec p5ag_topk_accept_boundary_runtime" in line:
            return line
    return None


def probe_p5ag_topk_accept_boundary_trace(trace_log: pathlib.Path | None = None) -> dict[str, Any]:
    errors: list[str] = []
    ...
    if trace_log is not None:
        if not trace_log.exists():
            ...
            if line is None:
                errors.append("trace log does not contain draft-jetspec p5ag_topk_accept_boundary_runtime")
            else:
                live_runtime = True
    ...
    return {
        "ok": not errors,
        "status": "p5ag_topk_accept_boundary_trace_contract_verified" if not errors else "p5ag_topk_accept_boundary_trace_contract_invalid",
        "errors": errors,
        ...
        "limitations": [
            ...
            "accept boundary has no logits or real target accept decision",
            "does not execute draft-head graph, commit, KV mutation, publish, or draft tokens",
        ],
    }
...
def main(argv: list[str]) -> int:
    ...
    out = probe_p5ag_topk_accept_boundary_trace(args.trace_log)
