#!/usr/bin/env python3
"""Validate the P5W JetSpec publish-gate descriptor source slice."""
...
REQUIRED_TOKENS: dict[pathlib.Path, list[str]] = {
    pathlib.Path("common/speculative.cpp"): ['JETSPEC_PUBLISH_GATE_PHASE', 'JETSPEC_PUBLISH_GATE_DESCRIPTOR', 'publish_post_commit_state', 'publish_gate_descriptor_only', 'publish_gate_descriptor_ready', 'invalid_publish_gate_descriptor', 'n_publish_gate_descriptors', 'publish_gate_descriptor_hash_last', 'build_publish_gate_descriptor', '!rejected_branch_discard_descriptor_ready || rejected_branch_discard_descriptor_hash_last == 0', 'actual_publish_visible_state_last = 0', 'common_speculative_fnv1a64(JETSPEC_PUBLISH_GATE_PHASE', 'disable_runtime_state(jetspec_runtime_failure::invalid_publish_gate_descriptor', 'publish_gate_descriptor_ready=%d', 'publish_gate_descriptor_hash=%016', 'publish_post_commit_state', 'actual_publish_visible_state=0', 'publish_after_commit_and_discard_only=1', 'publish_gate_descriptor_only=1', 'no_real_publish=1', 'no_visible_state_change=1', 'no_draft_tokens=1', 'publish_gate_phase=publish_post_commit_state'],
    pathlib.Path("docs/speculative.md"): ['P5W publish-gate descriptor', 'publish_post_commit_state', 'publish-gate descriptor', 'actual_publish_visible_state=0', 'publish_after_commit_and_discard_only=1', 'no real publish', 'no visible state change', 'no draft tokens', 'no draft tokens'],
    pathlib.Path("experiments/jetspec/jetspec_p5w_publish_gate_descriptor_candidate.md"): ['JetSpec P5W publish-gate descriptor candidate', 'approved bounded production-source slice', 'default-off and non-drafting', 'common/speculative.cpp', 'docs/speculative.md', 'publish_gate_descriptor_ready', 'invalid_publish_gate_descriptor', 'actual_publish_visible_state=0', 'publish_after_commit_and_discard_only=1', 'no real publish', 'no visible state change', 'no draft tokens', 'no draft-head graph execution', 'no draft tokens emitted', 'no real KV mutation', 'no CUDA dispatch', 'no server route', 'no public API', 'no CMake wiring'],
}

P5_TOKENS = ['P5W publish-gate descriptor', 'jetspec_p5w_publish_gate_descriptor', 'validate_p5w_publish_gate_descriptor', 'test_p5w_publish_gate_descriptor', 'JETSPEC_PUBLISH_GATE_PHASE', 'JETSPEC_PUBLISH_GATE_DESCRIPTOR', 'publish_gate_descriptor_ready', 'invalid_publish_gate_descriptor', 'publish_gate_descriptor_hash_last']
...
def validate_p5w_publish_gate_descriptor() -> dict[str, Any]:
    errors: list[str] = []
    ...
    if "build_publish_gate_descriptor()" not in impl:
        errors.append("descriptor builder is missing")
    for token in ['actual_publish_visible_state_last']:
        if f"{token} = 0" not in impl:
            errors.append(f"descriptor must keep {token} at zero")
    if "disable_runtime_state(jetspec_runtime_failure::invalid_publish_gate_descriptor" not in impl:
    ...
    for token in ['actual_publish_visible_state=0', 'publish_after_commit_and_discard_only=1', 'no real publish', 'no visible state change', 'no draft tokens', 'no draft tokens']:
    ...
    return {
        "ok": not errors,
        "errors": errors,
        "status": "p5w_publish_gate_descriptor_validated" if not errors else "p5w_publish_gate_descriptor_invalid",
...
def main(argv: list[str]) -> int:
    parse_args(argv)
    out = validate_p5w_publish_gate_descriptor()
