#!/usr/bin/env python3
"""Validate the P5S JetSpec accept-path descriptor source slice."""
...
REQUIRED_TOKENS: dict[pathlib.Path, list[str]] = {
    pathlib.Path("common/speculative.cpp"): ['JETSPEC_ACCEPT_PATH_PHASE', 'JETSPEC_ACCEPT_PATH_ROLLBACK_POINT', 'JETSPEC_ACCEPT_PATH_DESCRIPTOR', 'accept_path', 'after_accept', 'accept_path_descriptor_only', 'accept_path_descriptor_ready', 'invalid_accept_path_descriptor', 'n_accept_path_descriptors', 'accept_path_descriptor_hash_last', 'build_accept_path_descriptor', '!verify_mask_descriptor_ready || verify_mask_descriptor_hash_last == 0', 'actual_accepted_nodes_last = 0', 'correction_token_present_last = 0', 'common_speculative_fnv1a64(JETSPEC_ACCEPT_PATH_PHASE', 'common_speculative_fnv1a64(JETSPEC_ACCEPT_PATH_ROLLBACK_POINT', 'disable_runtime_state(jetspec_runtime_failure::invalid_accept_path_descriptor', 'accept_path_descriptor_ready=%d', 'accept_path_descriptor_hash=%016', 'accept_path', 'actual_accepted_nodes=0', 'correction_token_present=0', 'accept_path_descriptor_only=1', 'no_real_accept=1', 'no_commit_tokens=1', 'no_kv_mutation=1', 'no_publish=1', 'no_draft_tokens=1', 'accept_path_phase=accept_path', 'rollback_point=after_accept'],
    pathlib.Path("docs/speculative.md"): ['P5S accept-path descriptor', 'accept_path', 'accept-path descriptor', 'actual_accepted_nodes=0', 'correction_token_present=0', 'no real accept', 'no target logits walk', 'no token commit', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft tokens'],
    pathlib.Path("experiments/jetspec/jetspec_p5s_accept_path_descriptor_candidate.md"): ['JetSpec P5S accept-path descriptor candidate', 'approved bounded production-source slice', 'default-off and non-drafting', 'common/speculative.cpp', 'docs/speculative.md', 'accept_path_descriptor_ready', 'invalid_accept_path_descriptor', 'actual_accepted_nodes=0', 'correction_token_present=0', 'no real accept', 'no target logits walk', 'no token commit', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft-head graph execution', 'no draft tokens emitted', 'no real KV mutation', 'no CUDA dispatch', 'no server route', 'no public API', 'no CMake wiring'],
}

P5_TOKENS = ['P5S accept-path descriptor', 'jetspec_p5s_accept_path_descriptor', 'validate_p5s_accept_path_descriptor', 'test_p5s_accept_path_descriptor', 'JETSPEC_ACCEPT_PATH_PHASE', 'JETSPEC_ACCEPT_PATH_ROLLBACK_POINT', 'JETSPEC_ACCEPT_PATH_DESCRIPTOR', 'accept_path_descriptor_ready', 'invalid_accept_path_descriptor', 'accept_path_descriptor_hash_last']
...
def validate_p5s_accept_path_descriptor() -> dict[str, Any]:
    errors: list[str] = []
    ...
    if "build_accept_path_descriptor()" not in impl:
        errors.append("descriptor builder is missing")
    for token in ['actual_accepted_nodes_last', 'correction_token_present_last']:
        if f"{token} = 0" not in impl:
            errors.append(f"descriptor must keep {token} at zero")
    if "disable_runtime_state(jetspec_runtime_failure::invalid_accept_path_descriptor" not in impl:
    ...
    for token in ['actual_accepted_nodes=0', 'correction_token_present=0', 'no real accept', 'no target logits walk', 'no token commit', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft tokens']:
    ...
    return {
        "ok": not errors,
        "errors": errors,
        "status": "p5s_accept_path_descriptor_validated" if not errors else "p5s_accept_path_descriptor_invalid",
...
def main(argv: list[str]) -> int:
    parse_args(argv)
    out = validate_p5s_accept_path_descriptor()
