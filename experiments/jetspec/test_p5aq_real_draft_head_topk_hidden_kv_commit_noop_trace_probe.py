#!/usr/bin/env python3
"""Tests for P5AQ real draft-head top-k hidden/KV commit no-op trace probe and validator."""
...
import probe_p5aq_real_draft_head_topk_hidden_kv_commit_noop_trace as probe
import validate_p5aq_real_draft_head_topk_hidden_kv_commit_noop_runtime as validator
...
class P5AQRealDraftHeadTopKHiddenKVCommitNoopTraceProbeTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_probe_passes_without_live_log(self) -> None:
        out = probe.probe_p5aq_real_draft_head_topk_hidden_kv_commit_noop_trace()
        self.assertTrue(out["ok"], out["errors"])
        self.assertEqual(out["status"], "p5aq_real_draft_head_topk_hidden_kv_commit_noop_trace_contract_verified")
    ...
    def test_trace_line_requires_hidden_kv_noop_and_zero_side_effects(self) -> None:
        ...
        self.assertEqual(parsed["reuse_p5ap_token_commit_noop"], 1)
    ...
    def test_trace_line_rejects_duplicated_nonzero_side_effects(self) -> None:
        bad = (
            ...
            + " token_commit_noop=0 hidden_kv_commit_noop=0 reuse_p5ap_token_commit_noop=0 no_target_logits_walk=0 no_target_accept_walk=0 no_accept=0"
        ...
        for token in [
            ...
            "actual_publish_visible_state=1", "token_commit_noop=0", "hidden_kv_commit_noop=0", "reuse_p5ap_token_commit_noop=0",
    ...
    def test_trace_line_rejects_duplicate_nonzero_side_effect_counters_above_one(self) -> None:
        bad = (
            ...
            + " actual_pages_discarded=2 actual_publish_visible_state=2"
        )
        parsed = probe.validate_trace_line(bad)
        self.assertFalse(parsed["ok"])
        joined = "\n".join(parsed["errors"])
        for key in [
            ...
            self.assertIn(f"P5AQ trace {key} must appear exactly once with value 0", joined)
            self.assertIn(f"P5AQ trace {key} must be 0", joined)
    ...
    def test_trace_line_rejects_forbidden_readiness_and_generation_stats(self) -> None:
        bad = (
            ...
            + " rejected_branch_discard_descriptor_ready=1 publish_gate_descriptor_ready=1 token_commit_descriptor_ready=1"
    ...
    def test_probe_accepts_live_trace_log(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out = probe.probe_p5aq_real_draft_head_topk_hidden_kv_commit_noop_trace(path)
        self.assertTrue(out["ok"], out["errors"])
        ...
        self.assertEqual(out["live_trace"]["actual_survivor_pages_committed"], 0)

    def test_probe_rejects_missing_live_trace(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = pathlib.Path(tmp) / "server.log"
            path.write_text("no p5aq trace here\n", encoding="utf-8")
            out = probe.probe_p5aq_real_draft_head_topk_hidden_kv_commit_noop_trace(path)
        self.assertFalse(out["ok"])
        self.assertIn("trace log does not contain draft-jetspec p5aq_real_draft_head_topk_hidden_kv_commit_noop_runtime", out["errors"])
    ...
    def test_validator_passes(self) -> None:
        out = validator.validate_p5aq_real_draft_head_topk_hidden_kv_commit_noop_runtime()
        self.assertTrue(out["ok"], out["errors"])
        self.assertEqual(out["status"], "p5aq_real_draft_head_topk_hidden_kv_commit_noop_contract_wiring_validated")
        self.assertFalse(out["runtime_executed"])
        self.assertFalse(out["model_loaded"])
        self.assertFalse(out["draft_tokens_emitted"])
