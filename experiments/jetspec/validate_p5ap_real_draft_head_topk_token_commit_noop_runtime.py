#!/usr/bin/env python3
"""Validate the P5AP real draft-head top-k token-commit no-op no-model contract wiring."""
...
PROBE = HERE / "probe_p5ap_real_draft_head_topk_token_commit_noop_trace.py"
TEST = HERE / "test_p5ap_real_draft_head_topk_token_commit_noop_trace_probe.py"
...
P5AP_TOKENS = [
    ...
    "real_topk_token_commit_noop_runtime_ready",
]
...
REQUIRED_TEST_TOKENS = [
    "P5APRealDraftHeadTopKTokenCommitNoopTraceProbeTests",
    "test_probe_passes_without_live_log",
    "test_trace_line_requires_token_commit_noop_and_zero_side_effects",
    "test_parse_helpers_require_key_boundaries",
    "test_trace_line_rejects_missing_required_zero_counters",
    "test_trace_line_rejects_nonzero_side_effects",
    "test_trace_line_rejects_generic_root_and_downstream_readiness",
...
REQUIRED_AGGREGATE_TOKENS = [
    ...
    "validate_p5ap_real_draft_head_topk_token_commit_noop_runtime.py",
    "probe_p5ap_real_draft_head_topk_token_commit_noop_trace.py",
    "p5ap_real_draft_head_topk_token_commit_noop_trace_contract_verified",
    "LLAMA_JETSPEC_REAL_DRAFT_HEAD_TOPK_TOKEN_COMMIT_NOOP_ABI_ONLY",
    "invalid_real_draft_head_topk_token_commit_noop_runtime",
]
...
CMAKE_FORBIDDEN_TOKENS = P5AP_TOKENS + [
    ...
    for root in FORBIDDEN_WIRING_ROOTS:
        ...
        for path in base.rglob("*"):
            if not path.is_file() or path.suffix not in suffixes:
                continue
            text = _read(path)
            matched = [token for token in P5AP_TOKENS if token in text]
            if matched:
...
def _source_hook_errors(production_source: str, common_hits: list[dict[str, Any]]) -> list[str]:
    ...
    for hit in bad_common:
        errors.append(f"P5AP source hook must be limited to common/speculative.cpp: {hit}")
    if "p5ap_real_draft_head_topk_token_commit_noop_runtime" in production_source:
        ...
        if "hidden_kv_survivor_commit_descriptor_ready=1" in production_source:
            errors.append("P5AP source hook must return before downstream hidden/KV commit readiness")
        if "rejected_branch_discard_descriptor_ready=1" in production_source:
            errors.append("P5AP source hook must return before downstream rejected-branch discard readiness")
        if "publish_gate_descriptor_ready=1" in production_source:
            errors.append("P5AP source hook must return before downstream publish readiness")
    return errors


def validate_p5ap_real_draft_head_topk_token_commit_noop_runtime() -> dict[str, Any]:
    errors: list[str] = []
    doc = _read(DOC)
    readme = _read(README)
    ...
    for token in FORBIDDEN_CONTRACT_TOKENS:
        if token in contract_text:
            errors.append(f"P5AP no-model contract must not contain runtime side-effect token: {token}")
    ...
    for hit in cmake_hits:
        errors.append(f"P5AP contract must not add CMake wiring: {hit}")
    for hit in wiring_hits:
        errors.append(f"P5AP contract must not add server/public API/ggml wiring: {hit}")

    return {
        "ok": not errors,
        "status": "p5ap_real_draft_head_topk_token_commit_noop_contract_wiring_validated" if not errors else "p5ap_real_draft_head_topk_token_commit_noop_contract_wiring_invalid",
        "errors": errors,
        "runtime_executed": False,
        "model_loaded": False,
        "context_created": False,
        "source_hook_limited_to_common_speculative_cpp": not any(hit["path"] != "common/speculative.cpp" for hit in common_hits),
        "source_implementation_present": "p5ap_real_draft_head_topk_token_commit_noop_runtime" in production_source,
...
def main(argv: list[str]) -> int:
    ...
    out = validate_p5ap_real_draft_head_topk_token_commit_noop_runtime()
