#!/usr/bin/env python3
"""Validate the P5R JetSpec verify-mask descriptor source slice."""
...
REQUIRED_TOKENS: dict[pathlib.Path, list[str]] = {
    pathlib.Path("common/speculative.cpp"): ['JETSPEC_VERIFY_MASK_PHASE', 'JETSPEC_VERIFY_MASK_ROLLBACK_POINT', 'JETSPEC_VERIFY_MASK_DESCRIPTOR', 'build_verify_mask', 'after_verify_mask', 'verify_mask_descriptor_only', 'verify_mask_descriptor_ready', 'invalid_verify_mask_descriptor', 'n_verify_mask_descriptors', 'verify_mask_descriptor_hash_last', 'build_verify_mask_descriptor', '!tree_build_descriptor_ready || tree_build_descriptor_hash_last == 0', 'actual_verify_mask_entries_last = 0', 'common_speculative_fnv1a64(JETSPEC_VERIFY_MASK_PHASE', 'common_speculative_fnv1a64(JETSPEC_VERIFY_MASK_ROLLBACK_POINT', 'disable_runtime_state(jetspec_runtime_failure::invalid_verify_mask_descriptor', 'verify_mask_descriptor_ready=%d', 'verify_mask_descriptor_hash=%016', 'build_verify_mask', 'actual_verify_mask_entries=0', 'verify_mask_descriptor_only=1', 'no_real_verify_mask=1', 'no_verify_mask=1', 'no_accept=1', 'no_kv_mutation=1', 'no_publish=1', 'no_draft_tokens=1', 'verify_mask_phase=build_verify_mask', 'rollback_point=after_verify_mask'],
    pathlib.Path("docs/speculative.md"): ['P5R verify-mask descriptor', 'build_verify_mask', 'verify-mask descriptor', 'actual_verify_mask_entries=0', 'no real verify mask', 'no mask tensor', 'no accept', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft tokens'],
    pathlib.Path("experiments/jetspec/jetspec_p5r_verify_mask_descriptor_candidate.md"): ['JetSpec P5R verify-mask descriptor candidate', 'approved bounded production-source slice', 'default-off and non-drafting', 'common/speculative.cpp', 'docs/speculative.md', 'verify_mask_descriptor_ready', 'invalid_verify_mask_descriptor', 'actual_verify_mask_entries=0', 'no real verify mask', 'no mask tensor', 'no accept', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft-head graph execution', 'no draft tokens emitted', 'no real KV mutation', 'no CUDA dispatch', 'no server route', 'no public API', 'no CMake wiring'],
}

P5_TOKENS = ['P5R verify-mask descriptor', 'jetspec_p5r_verify_mask_descriptor', 'validate_p5r_verify_mask_descriptor', 'test_p5r_verify_mask_descriptor', 'JETSPEC_VERIFY_MASK_PHASE', 'JETSPEC_VERIFY_MASK_ROLLBACK_POINT', 'JETSPEC_VERIFY_MASK_DESCRIPTOR', 'verify_mask_descriptor_ready', 'invalid_verify_mask_descriptor', 'verify_mask_descriptor_hash_last']
...
def validate_p5r_verify_mask_descriptor() -> dict[str, Any]:
    errors: list[str] = []
    ...
    if "build_verify_mask_descriptor()" not in impl:
        errors.append("descriptor builder is missing")
    for token in ['actual_verify_mask_entries_last']:
        if f"{token} = 0" not in impl:
            errors.append(f"descriptor must keep {token} at zero")
    if "disable_runtime_state(jetspec_runtime_failure::invalid_verify_mask_descriptor" not in impl:
    ...
    for token in ['actual_verify_mask_entries=0', 'no real verify mask', 'no mask tensor', 'no accept', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft tokens']:
    ...
    return {
        "ok": not errors,
        "errors": errors,
        "status": "p5r_verify_mask_descriptor_validated" if not errors else "p5r_verify_mask_descriptor_invalid",
...
def main(argv: list[str]) -> int:
    parse_args(argv)
    out = validate_p5r_verify_mask_descriptor()
