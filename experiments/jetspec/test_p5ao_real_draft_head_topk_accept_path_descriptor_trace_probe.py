#!/usr/bin/env python3
"""Tests for P5AO real draft-head top-k accept-path descriptor trace probe and validator."""
...
import probe_p5ao_real_draft_head_topk_accept_path_descriptor_trace as probe
import validate_p5ao_real_draft_head_topk_accept_path_descriptor_runtime as validator
...
class P5AORealDraftHeadTopKAcceptPathDescriptorTraceProbeTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_probe_passes_without_live_log(self) -> None:
        out = probe.probe_p5ao_real_draft_head_topk_accept_path_descriptor_trace()
        self.assertTrue(out["ok"], out["errors"])
        self.assertEqual(out["status"], "p5ao_real_draft_head_topk_accept_path_descriptor_trace_contract_verified")
    ...
    def test_trace_line_requires_descriptor_and_zero_side_effects(self) -> None:
        ...
        self.assertEqual(parsed["descriptor_only"], 1)
        self.assertEqual(parsed["reuse_p5an_accept_boundary_metadata"], 1)
        ...
        for key in [
            ...
            "no_hidden_kv_commit", "no_rejected_branch_discard", "no_publish",
    ...
    def test_trace_line_rejects_nonzero_side_effects(self) -> None:
        bad = (
            ...
            + " no_target_logits_walk=0 no_target_accept_walk=0 no_accept=0 no_token_commit=0 no_hidden_kv_commit=0"
            + " no_rejected_branch_discard=0 no_publish=0 no_visible_state_change=0 no_kv_mutation=0 no_draft_tokens=0"
        )
        parsed = probe.validate_trace_line(bad)
        self.assertFalse(parsed["ok"])
        joined = "\n".join(parsed["errors"])
        for token in [
            ...
            "actual_publish_visible_state=1", "no_token_commit=0", "no_hidden_kv_commit=0",
            "no_rejected_branch_discard=0", "no_publish=0", "no_kv_mutation=0", "no_draft_tokens=0",
        ]:
            self.assertIn(token, joined)

    def test_trace_line_rejects_token_commit_hidden_kv_discard_publish_readiness(self) -> None:
        bad = (
            probe.SELF_TEST_TRACE
            + " token_commit_descriptor_ready=1 hidden_kv_survivor_commit_descriptor_ready=1"
            + " rejected_branch_discard_descriptor_ready=1 publish_gate_descriptor_ready=1"
        )
        parsed = probe.validate_trace_line(bad)
        self.assertFalse(parsed["ok"])
        joined = "\n".join(parsed["errors"])
        self.assertIn("token_commit_descriptor_ready=1", joined)
        self.assertIn("hidden_kv_survivor_commit_descriptor_ready=1", joined)
        self.assertIn("rejected_branch_discard_descriptor_ready=1", joined)
        self.assertIn("publish_gate_descriptor_ready=1", joined)
    ...
    def test_probe_accepts_live_trace_log(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out = probe.probe_p5ao_real_draft_head_topk_accept_path_descriptor_trace(path)
        self.assertTrue(out["ok"], out["errors"])
        ...
        self.assertEqual(out["live_trace"]["accept_path_descriptor_len"], 0)

    def test_probe_rejects_missing_live_trace(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = pathlib.Path(tmp) / "server.log"
            path.write_text("no p5ao trace here\n", encoding="utf-8")
            out = probe.probe_p5ao_real_draft_head_topk_accept_path_descriptor_trace(path)
        self.assertFalse(out["ok"])
        self.assertIn("trace log does not contain draft-jetspec p5ao_real_draft_head_topk_accept_path_descriptor_runtime", out["errors"])
    ...
    def test_validator_passes(self) -> None:
        out = validator.validate_p5ao_real_draft_head_topk_accept_path_descriptor_runtime()
        self.assertTrue(out["ok"], out["errors"])
        self.assertEqual(out["status"], "p5ao_real_draft_head_topk_accept_path_descriptor_contract_wiring_validated")
        self.assertFalse(out["runtime_executed"])
        self.assertFalse(out["model_loaded"])
        self.assertFalse(out["draft_tokens_emitted"])
