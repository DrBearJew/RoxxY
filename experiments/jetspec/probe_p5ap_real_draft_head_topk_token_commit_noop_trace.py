#!/usr/bin/env python3
"""Fast no-model P5AP real draft-head top-k token-commit no-op ABI trace probe."""
...
REQUIRED_TRACE_TOKENS = [
    "draft-jetspec p5ap_real_draft_head_topk_token_commit_noop_runtime",
...
FORBIDDEN_TRACE_TOKENS = [
    ...
    "token_commit_noop=0",
    "reuse_p5ao_accept_path_descriptor=0",
...
SELF_TEST_TRACE = (
    "common_speculative_impl_draft_jetspec::process: draft-jetspec p5ap_real_draft_head_topk_token_commit_noop_runtime "
    "phase=real_draft_head_topk_token_commit_noop_ready real_topk_token_commit_noop_runtime_ready=1 "
    ...
    "accept_decision_source=none_no_target_logits token_commit_noop=1 reuse_p5ao_accept_path_descriptor=1 "
    ...
    "no_real_token_commit=1 no_visible_token_publish=1 no_hidden_kv_commit=1 no_rejected_branch_discard=1 "
...
def validate_trace_line(line: str) -> dict[str, Any]:
    ...
    errors.extend(f"missing P5AP trace token: {token}" for token in missing)
    errors.extend(f"forbidden P5AP trace token present: {token}" for token in forbidden)
    ...
    scalar_keys = [
        ...
        "token_commit_noop", "reuse_p5ao_accept_path_descriptor", "actual_committed_tokens",
        ...
        "no_real_token_commit", "no_visible_token_publish", "no_hidden_kv_commit", "no_rejected_branch_discard",
        "no_publish", "no_visible_state_change", "no_kv_mutation", "no_draft_tokens",
    ]
    ...
    for key, value in expected_exact.items():
        if parsed[key] != value:
            errors.append(f"P5AP trace {key} must be {value}")
    ...
    if candidate_ids is None or len(candidate_ids) != 2 or candidate_ids[0] < 0 or candidate_ids[1] < 0 or candidate_ids[0] == candidate_ids[1]:
        errors.append("P5AP trace candidate_ids must contain two distinct non-negative ids")
    if tree_ids is None or len(tree_ids) != 3:
        errors.append("P5AP trace real_tree_token_ids must contain root plus two candidates")
    elif candidate_ids is not None and tree_ids[1:] != candidate_ids:
        errors.append("P5AP trace real_tree_token_ids[1:] must equal P5AK candidate_ids")
    ...
    for path in [DOC, README, CANDIDATE]:
        ...
        for token in [
            ...
            if token not in text:
                errors.append(f"{path.name} missing P5AP contract token: {token}")
...
def _trace_from_log(path: pathlib.Path) -> str | None:
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        if "draft-jetspec p5ap_real_draft_head_topk_token_commit_noop_runtime" in line:
            return line
    return None


def probe_p5ap_real_draft_head_topk_token_commit_noop_trace(trace_log: pathlib.Path | None = None) -> dict[str, Any]:
    errors: list[str] = []
    ...
    if trace_log is not None:
        if not trace_log.exists():
            ...
            if line is None:
                errors.append("trace log does not contain draft-jetspec p5ap_real_draft_head_topk_token_commit_noop_runtime")
            else:
                live_runtime = True
    ...
    return {
        "ok": not errors,
        "status": "p5ap_real_draft_head_topk_token_commit_noop_trace_contract_verified" if not errors else "p5ap_real_draft_head_topk_token_commit_noop_trace_contract_invalid",
        "errors": errors,
        ...
        "limitations": [
            ...
            "P5AP reuses P5AO accept-path descriptor metadata to describe a token-commit no-op",
            "does not commit tokens, publish visible tokens, mutate hidden/KV, discard pages, publish visible state, or emit draft tokens",
        ],
    }
...
def main(argv: list[str]) -> int:
    ...
    out = probe_p5ap_real_draft_head_topk_token_commit_noop_trace(args.trace_log)
