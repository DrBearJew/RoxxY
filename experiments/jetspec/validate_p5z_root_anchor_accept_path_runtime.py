#!/usr/bin/env python3
"""Validate the P5Z JetSpec root-anchor accept-path runtime source slice."""
...
REQUIRED_TOKENS: dict[pathlib.Path, list[str]] = {
    pathlib.Path("common/speculative.cpp"): [
        ...
        "root_anchor_accept_path_runtime",
        ...
        "root_verified_anchor_last",
        "accept_path_len_last",
        "build_root_anchor_accept_path_runtime",
        "p5z_root_anchor_accept_path_enabled && (!p5x_root_tree_enabled || !p5y_root_verify_mask_enabled)",
        "!p5y_root_verify_mask_enabled || !root_verify_mask_runtime_ready || root_verify_mask_runtime_hash_last == 0",
        "!p5x_root_tree_enabled || !root_tree_runtime_ready || root_tree_runtime_hash_last == 0",
        "tree_build_actual_nodes_last != 1 || actual_verify_mask_entries_last != 1",
        ...
        "n_root_anchor_accept_path_runtime_builds++",
        "disable_runtime_state(jetspec_runtime_failure::invalid_root_anchor_accept_path_runtime",
    ...
    pathlib.Path("experiments/jetspec/jetspec_p5z_root_anchor_accept_path_runtime_candidate.md"): [
...
class P5ZRootAnchorAcceptPathRuntimeError(ValueError):
    """Raised when P5Z source-slice constraints are violated."""


def _read(rel: pathlib.Path) -> str:
    path = REPO_ROOT / rel
    if not path.exists():
        raise P5ZRootAnchorAcceptPathRuntimeError(f"missing required file: {rel}")
...
def _impl_slice(source: str) -> str:
    ...
    if start < 0 or end < 0 or end <= start:
        raise P5ZRootAnchorAcceptPathRuntimeError("cannot isolate draft-jetspec implementation slice")
    return source[start:end]


def _p5z_branch(impl: str) -> str:
    start = impl.find("if (p5z_root_anchor_accept_path_enabled) {\n                    if (!build_root_anchor_accept_path_runtime())")
    end = impl.find("                if (trace_taps) {\n                    LOG_INF(\"%s: draft-jetspec p5y_root_verify_mask_runtime", start)
    if start < 0 or end < 0 or end <= start:
        raise P5ZRootAnchorAcceptPathRuntimeError("cannot isolate P5Z branch before P5T token commit")
    return impl[start:end]


def _dirty_p5z_hits() -> list[dict[str, Any]]:
    try:
    ...
    for line in proc.stdout.splitlines():
        ...
        if not rel or rel in P5Z_ALLOWED_FILES or str(rel).startswith("experiments/jetspec/"):
            continue
        ...
        for token in P5Z_TOKENS:
            if token in diff:
                hits.append({"path": str(rel), "token": token})
    return hits


def validate_p5z_root_anchor_accept_path_runtime() -> dict[str, Any]:
    errors: list[str] = []

    for rel, tokens in REQUIRED_TOKENS.items():
        try:
            text = _read(rel)
        except P5ZRootAnchorAcceptPathRuntimeError as exc:
    ...
    for rel in _iter_source_files():
        ...
        for token in P5Z_TOKENS:
            if token in text:
                ...
                if rel not in P5Z_ALLOWED_FILES:
                    errors.append(f"P5Z token {token!r} appears outside allowlist: {rel}")
    ...
    for rel in _cmake_files():
        ...
        if matched:
            ...
            errors.append(f"P5Z must not add explicit CMake wiring in {rel}: {matched}")

    dirty_p5z_hits = _dirty_p5z_hits()
    for hit in dirty_p5z_hits:
        errors.append(f"dirty diff contains P5Z token outside allowlist: {hit}")

    try:
        source = _read(pathlib.Path("common/speculative.cpp"))
        impl = _impl_slice(source)
        branch = _p5z_branch(impl)
    except P5ZRootAnchorAcceptPathRuntimeError as exc:
        errors.append(str(exc))
        impl = ""
        branch = ""
    ...
    for token in FORBIDDEN_IMPL_TOKENS:
        if token in impl:
            errors.append(f"P5Z implementation must not contain {token!r}")
    for token in FORBIDDEN_P5Z_BRANCH_TOKENS:
        if token in branch:
            errors.append(f"P5Z branch must return before {token}")
    if branch and "return true;" not in branch:
        errors.append("P5Z branch must return before P5T token commit")
    if branch and "build_accept_path_descriptor()" in branch:
        errors.append("P5Z must build the root-anchor runtime object, not call the P5S descriptor builder")
    if "runtime_supported=true" in impl:
        errors.append("P5Z must not claim runtime_supported=true")
    if re.search(r"common_sampler_sample|llama_sampler", impl):
        errors.append("P5Z must not sample draft-head logits")
    ...
    for token in [
        ...
        if token not in docs:
            errors.append(f"docs/speculative.md missing P5Z boundary token: {token}")

    return {
        "ok": not errors,
        "errors": errors,
        "status": "p5z_root_anchor_accept_path_runtime_validated" if not errors else "p5z_root_anchor_accept_path_runtime_invalid",
        "allowed_files": sorted(str(path) for path in P5Z_ALLOWED_FILES),
        "token_hits": token_hits,
        "cmake_hits": cmake_hits,
        "dirty_p5z_hits": dirty_p5z_hits,
    }
...
def main(argv: list[str]) -> int:
    ...
    out = validate_p5z_root_anchor_accept_path_runtime()
    if args.json:
        print(json.dumps(out, indent=2, sort_keys=True))
    elif out["ok"]:
        print("P5Z root-anchor accept-path runtime validation passed")
    else:
        print("P5Z root-anchor accept-path runtime validation failed", file=sys.stderr)
