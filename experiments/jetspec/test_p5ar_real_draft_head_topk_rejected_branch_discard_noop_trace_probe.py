#!/usr/bin/env python3
"""Tests for P5AR real draft-head top-k rejected-branch discard no-op trace probe and validator."""
...
import probe_p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace as probe
import validate_p5ar_real_draft_head_topk_rejected_branch_discard_noop_runtime as validator
...
class P5ARRealDraftHeadTopKRejectedBranchDiscardNoopTraceProbeTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_probe_passes_without_live_log(self) -> None:
        out = probe.probe_p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace()
        self.assertTrue(out["ok"], out["errors"])
        self.assertEqual(out["status"], "p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace_contract_verified")
    ...
    def test_trace_line_requires_discard_noop_and_zero_side_effects(self) -> None:
        ...
        self.assertEqual(parsed["reuse_p5aq_hidden_kv_commit_noop"], 1)
    ...
    def test_trace_line_rejects_duplicated_nonzero_side_effects(self) -> None:
        bad = (
            ...
            + " token_commit_noop=0 hidden_kv_commit_noop=0 reuse_p5aq_hidden_kv_commit_noop=0 rejected_branch_discard_noop=0"
            ...
            + " no_real_rejected_branch_discard=0 no_rejected_branch_discard=0 no_publish=0 no_visible_state_change=0"
        ...
        for token in [
            ...
            "token_commit_noop=0", "hidden_kv_commit_noop=0", "reuse_p5aq_hidden_kv_commit_noop=0", "rejected_branch_discard_noop=0",
            "no_target_logits_walk=0", "no_target_accept_walk=0", "no_accept=0",
            "no_real_rejected_branch_discard=0", "no_rejected_branch_discard=0", "no_publish=0", "no_kv_mutation=0", "no_draft_tokens=0",
        ]:
    ...
    def test_trace_line_rejects_duplicate_nonzero_side_effect_counters_above_one(self) -> None:
        ...
        for key in [
            ...
            self.assertIn(f"P5AR trace {key} must appear exactly once with value 0", joined)
            self.assertIn(f"P5AR trace {key} must be 0", joined)
    ...
    def test_trace_line_rejects_forbidden_readiness_and_generation_stats(self) -> None:
        bad = (
            ...
            + " publish_gate_descriptor_ready=1 root_publish_gate_noop_runtime_ready=1 token_commit_descriptor_ready=1"
            + " root_token_commit_noop_runtime_ready=1 #gen drafts = 1 #gen tokens = 1"
        )
        parsed = probe.validate_trace_line(bad)
        self.assertFalse(parsed["ok"])
        joined = "\n".join(parsed["errors"])
    ...
    def test_probe_accepts_live_trace_log(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out = probe.probe_p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace(path)
        self.assertTrue(out["ok"], out["errors"])
        ...
        self.assertEqual(out["live_trace"]["actual_pages_discarded"], 0)
        self.assertEqual(out["live_trace"]["rejected_branch_pages_reachable_after_discard"], 0)

    def test_probe_rejects_missing_live_trace(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = pathlib.Path(tmp) / "server.log"
            path.write_text("no p5ar trace here\n", encoding="utf-8")
            out = probe.probe_p5ar_real_draft_head_topk_rejected_branch_discard_noop_trace(path)
        self.assertFalse(out["ok"])
        self.assertIn("trace log does not contain draft-jetspec p5ar_real_draft_head_topk_rejected_branch_discard_noop_runtime", out["errors"])
    ...
    def test_validator_passes(self) -> None:
        out = validator.validate_p5ar_real_draft_head_topk_rejected_branch_discard_noop_runtime()
        self.assertTrue(out["ok"], out["errors"])
        self.assertEqual(out["status"], "p5ar_real_draft_head_topk_rejected_branch_discard_noop_contract_wiring_validated")
        self.assertFalse(out["runtime_executed"])
        self.assertFalse(out["model_loaded"])
        self.assertFalse(out["draft_tokens_emitted"])
        ...
        self.assertEqual(out["rejected_branch_pages_reachable_after_discard"], 0)
