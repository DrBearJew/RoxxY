#!/usr/bin/env python3
"""Validate the P5V JetSpec rejected-branch discard descriptor source slice."""
...
REQUIRED_TOKENS: dict[pathlib.Path, list[str]] = {
    pathlib.Path("common/speculative.cpp"): ['JETSPEC_REJECTED_BRANCH_DISCARD_PHASE', 'JETSPEC_REJECTED_BRANCH_DISCARD_ROLLBACK_POINT', 'JETSPEC_REJECTED_BRANCH_DISCARD_DESCRIPTOR', 'discard_rejected_branches', 'after_rejected_discard', 'rejected_branch_discard_descriptor_only', 'rejected_branch_discard_descriptor_ready', 'invalid_rejected_branch_discard_descriptor', 'n_rejected_branch_discard_descriptors', 'rejected_branch_discard_descriptor_hash_last', 'build_rejected_branch_discard_descriptor', '!hidden_kv_survivor_commit_descriptor_ready || hidden_kv_survivor_commit_descriptor_hash_last == 0', 'actual_pages_discarded_last = 0', 'rejected_branch_pages_reachable_after_discard_last = 0', 'common_speculative_fnv1a64(JETSPEC_REJECTED_BRANCH_DISCARD_PHASE', 'common_speculative_fnv1a64(JETSPEC_REJECTED_BRANCH_DISCARD_ROLLBACK_POINT', 'disable_runtime_state(jetspec_runtime_failure::invalid_rejected_branch_discard_descriptor', 'rejected_branch_discard_descriptor_ready=%d', 'rejected_branch_discard_descriptor_hash=%016', 'discard_rejected_branches', 'actual_pages_discarded=0', 'rejected_branch_pages_reachable_after_discard=0', 'rejected_branch_discard_descriptor_only=1', 'no_real_rejected_branch_discard=1', 'no_kv_mutation=1', 'no_publish=1', 'no_draft_tokens=1', 'rejected_branch_discard_phase=discard_rejected_branches', 'rollback_point=after_rejected_discard'],
    pathlib.Path("docs/speculative.md"): ['P5V rejected-branch discard descriptor', 'discard_rejected_branches', 'rejected-branch discard descriptor', 'actual_pages_discarded=0', 'rejected_branch_pages_reachable_after_discard=0', 'no real rejected-branch discard', 'rejected_branch_pages_reachable_after_discard=0', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft tokens'],
    pathlib.Path("experiments/jetspec/jetspec_p5v_rejected_branch_discard_descriptor_candidate.md"): ['JetSpec P5V rejected-branch discard descriptor candidate', 'approved bounded production-source slice', 'default-off and non-drafting', 'common/speculative.cpp', 'docs/speculative.md', 'rejected_branch_discard_descriptor_ready', 'invalid_rejected_branch_discard_descriptor', 'actual_pages_discarded=0', 'rejected_branch_pages_reachable_after_discard=0', 'no real rejected-branch discard', 'rejected_branch_pages_reachable_after_discard=0', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft-head graph execution', 'no draft tokens emitted', 'no real KV mutation', 'no CUDA dispatch', 'no server route', 'no public API', 'no CMake wiring'],
}

P5_TOKENS = ['P5V rejected-branch discard descriptor', 'jetspec_p5v_rejected_branch_discard_descriptor', 'validate_p5v_rejected_branch_discard_descriptor', 'test_p5v_rejected_branch_discard_descriptor', 'JETSPEC_REJECTED_BRANCH_DISCARD_PHASE', 'JETSPEC_REJECTED_BRANCH_DISCARD_ROLLBACK_POINT', 'JETSPEC_REJECTED_BRANCH_DISCARD_DESCRIPTOR', 'rejected_branch_discard_descriptor_ready', 'invalid_rejected_branch_discard_descriptor', 'rejected_branch_discard_descriptor_hash_last']
...
def validate_p5v_rejected_branch_discard_descriptor() -> dict[str, Any]:
    errors: list[str] = []
    ...
    if "build_rejected_branch_discard_descriptor()" not in impl:
        errors.append("descriptor builder is missing")
    for token in ['actual_pages_discarded_last', 'rejected_branch_pages_reachable_after_discard_last']:
        if f"{token} = 0" not in impl:
            errors.append(f"descriptor must keep {token} at zero")
    if "disable_runtime_state(jetspec_runtime_failure::invalid_rejected_branch_discard_descriptor" not in impl:
    ...
    for token in ['actual_pages_discarded=0', 'rejected_branch_pages_reachable_after_discard=0', 'no real rejected-branch discard', 'rejected_branch_pages_reachable_after_discard=0', 'no KV mutation', 'no publish', 'no draft tokens', 'no draft tokens']:
    ...
    return {
        "ok": not errors,
        "errors": errors,
        "status": "p5v_rejected_branch_discard_descriptor_validated" if not errors else "p5v_rejected_branch_discard_descriptor_invalid",
...
def main(argv: list[str]) -> int:
    parse_args(argv)
    out = validate_p5v_rejected_branch_discard_descriptor()
