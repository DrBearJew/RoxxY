...
> - 35B experimental ROCm MTP now auto-routes the pathological D=256 q8K/q4V-style long-fill/small-draft path to bounded f16-temp tile/MMA unless explicitly disabled with `GGML_CUDA_ROCM_QUANT_PREFILL_F16=0`.
> - On the requested IQ4_XS model (`Qwen3.6-35B-A3B-IQ4_XS`), `ctx=40960`, `fill=32768`, `pp=1024` improved from forced-off `1055 tok/s` fill and `536 tok/s` cached pp1024 to auto `2457 tok/s` fill and `1488 tok/s` cached pp1024 with `cache_n=32768` on `q8_0/tbq4_0`.
...
| `q8_0/q4_0` ROCm status | `--cache-type-k q8_0 --cache-type-v q4_0` | Not promoted over `q8_0/tbq4_0`, but 35B D=256 long-fill now uses the bounded f16-temp FA prefill route and outperformed the IQ4_XS Vulkan q8/q4 check. Forced-off VEC/non-FA remains available for A/B via `GGML_CUDA_ROCM_QUANT_PREFILL_F16=0`. |
