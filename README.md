...
> **2026-05-20 status update — fixes, speed, and the 35B MTP bug fix**
>
...
> - 35B experimental ROCm MTP now uses the bounded f16-temp tile/MMA path only through an explicit opt-in (`GGML_CUDA_ROCM_QUANT_PREFILL_F16=1`, or `_AUTO=1` for A/B probes). Absence of the f16 envs preserves q8K/tbq4V VEC so 35B prefill cannot silently sink into the bad path.
> - On the requested IQ4_XS model (`Qwen3.6-35B-A3B-IQ4_XS`), the temp-0.6 long-prose coherence canary at `ctx=40960`, explicit f16-stable, `q8_0/tbq4_0`, and MTP n2 passed: `prompt_n=36103` at `2032 tok/s`, `predicted_n=1505` at `58.5 tok/s`, peak delta from loaded `0.339 GiB`, route counts `tile:tile=3952`, `vec:q8k_tbq4v_vec=3211`, no repeated 120-char chunk. Artifact: `benches/rocm-rdna3/35b-iq4xs-coherence-temp06-32k-8k-f16fix-20260521-061142`.
> - ROCm q8/q4 now also takes the bounded f16-temp FA route for prefill: IQ4_XS `q8_0/q4_0` reached `2474 tok/s` fill and `1510 tok/s` cached pp1024 at ubatch 1024, with ubatch 2048 reaching `1518 tok/s` cached pp1024. This now beats the matching Vulkan/RADV IQ4_XS q8/q4 run (`1921 tok/s` fill, `1121 tok/s` cached pp1024). Earlier Q4_K_M validation measured f16 gate `1073 tok/s` fill and `1410 tok/s` cached pp1024 versus no-f16 `574 tok/s` fill and `536 tok/s` cached pp1024.
...
>
> **Remaining hardening roadmap:** keep `GGML_CUDA_ROCM_QUANT_PREFILL_F16=0` as the A/B escape hatch; keep testing the reduced no-MTP `q8_0/q4_0` / `q8_0/tbq4_0` ROCm prefill repro; and, longer term, either repair the underlying ROCm quantized-VEC depth scaling or retain this bounded f16-temp selector as the production mitigation. The fix gate is: no OOM, no request-time sleep, correct cache reuse, and 35B 32k-fill performance close to the Vulkan/f16 baselines.

The promoted ROCm path is **27B MTP long context** with the TurboQuant setting (`q8_0` K + `tbq4_0` V), plus a **35B MoE prompt-processing path** using the current best MMQ selector. 35B long-fill MTP uses the bounded selector fix described above only when explicitly opted in with `GGML_CUDA_ROCM_QUANT_PREFILL_F16=1`.

**Current default:** use `--cache-type-k q8_0 --cache-type-v tbq4_0` with VEC FlashAttention on ROCm. Keep `q8_0/tbq4_0` as the promoted ROCm TurboQuant setting. Do not generalize `q8_0/q4_0` as slow for every backend: Vulkan/RADV q8/q4 has been restored to upstream scalar/coopmat1 behavior and is healthy, and this branch now also supports the ROCm q8/q4 prefill case through bounded f16-temp tile/MMA. The ROCm issue is narrower: **35B long-fill/small-draft with quantized KV** scales pathologically on the ROCm VEC route, so this branch provides an explicit bounded f16-temp tile/MMA prefill opt-in for D=256 q8K/q4V-style prefill while preserving VEC decode and forced-off testing when the f16 envs are absent. Vulkan `q8_0/tbq4_0` still fails quality/perf gates and remains experimental. rocWMMA compressed-KV experiments are deprecated for now and should not be enabled in user-facing builds or wrappers. Short benchmark notes are kept near the bottom of this README.
...
| `q8_0/q4_0` ROCm status | `--cache-type-k q8_0 --cache-type-v q4_0` | Not promoted over `q8_0/tbq4_0`, but 35B D=256 long-fill can use the explicit bounded f16-temp FA prefill route and outperformed the IQ4_XS Vulkan q8/q4 check. Forced-off VEC/non-FA remains the default when `GGML_CUDA_ROCM_QUANT_PREFILL_F16` and `_AUTO` are absent. |
...
| 35B experimental MTP | add `--spec-type draft-mtp --spec-default --spec-draft-n-max 2 --spec-draft-p-min 0`, `LLAMA_MTP_PREFILL_CHUNK=1024 LLAMA_MTP_PREFILL_FORCE_MMQ=1`, and `GGML_CUDA_ROCM_QUANT_PREFILL_F16=1` | Explicitly opted-in workaround for 32k long-fill; IQ4_XS temp-0.6 coherence canary passed, but underlying ROCm quantized-KV FA depth-scaling bug remains open |
